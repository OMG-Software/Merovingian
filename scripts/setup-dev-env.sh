...
package_list_for() {
    case "$1" in
        apt)
            printf '%s\n' "build-essential clang lld meson ninja-build pkg-config git python3 perl bison flex m4 libssl-dev catch2 clang-format clang-tidy cppcheck"
            ;;
        dnf)
            printf '%s\n' "gcc-c++ clang lld meson ninja-build pkgconf-pkg-config git python3 perl bison flex m4 openssl-devel catch-devel clang-tools-extra cppcheck"
            ;;
        zypper)
            printf '%s\n' "gcc-c++ clang lld meson ninja pkg-config git python3 perl bison flex m4 libopenssl-devel catch2-devel clang-tools cppcheck"
            ;;
        pacman)
            printf '%s\n' "base-devel clang lld meson ninja pkgconf git python perl bison flex m4 openssl catch2 clang-tools-extra cppcheck"
            ;;
        apk)
            printf '%s\n' "build-base clang lld meson ninja pkgconf git python3 perl bison flex m4 openssl-dev catch2-dev clang-extra-tools cppcheck"
            ;;
        pkg)
            printf '%s\n' "llvm meson ninja pkgconf git python3 perl5 bison flex gmake openssl catch2 cppcheck"
            ;;
        pkg_add)
            printf '%s\n' "llvm meson ninja pkgconf git perl bison flex gmake openssl catch2 cppcheck"
            ;;
        pkgin)
            printf '%s\n' "clang meson ninja-build pkg-config git python311 perl bison flex gmake openssl catch2 cppcheck"
            ;;
        *)
