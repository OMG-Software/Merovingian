...
- **fix(build): explicit RELRO/BIND_NOW in meson.build and packaging scripts:** `meson.build` now includes `-Wl,-z,relro` and `-Wl,-z,now` in `hardening_link_flags` alongside the existing `-Wl,-z,noexecstack`; unsupported linkers will drop them via `get_supported_link_arguments`. Every packaging script (`scripts/build-deb.sh`, `scripts/build-rpm.sh`, `scripts/build-static-linux.sh`, `scripts/build-freebsd-pkg.sh`, `scripts/build-netbsd-pkg.sh`, `scripts/build-openbsd-pkg.sh`) and RPM spec (`packaging/rpm/merovingian.spec`, `packaging/rhel/merovingian.spec`, `packaging/opensuse/merovingian.spec`) passes these flags explicitly so dynamic package builds never omit them. `docs/hardening-alpha-exceptions.md` updated to retire `core dump policy`, `no_new_privs`, and `capability bounding` and to note the explicit linker-hardening flags.
