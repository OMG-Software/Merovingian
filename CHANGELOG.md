...
- **fix(ci): capture NetBSD core backtraces on test failure:** the `netbsd-build-and-test` job in `.github/workflows/ci.yml` now enables core dumps (`ulimit -c unlimited`, `kern.defcorename`), installs `gdb`, and on a failing `build-bsd.sh` run emits a `thread apply all bt` backtrace for every `*.core` under `build/`. This lets the roaming, nondeterministic unit-test `SIGABRT` on NetBSD be diagnosed from a real stack trace instead of by relocating `std::cerr` markers between scenarios.
