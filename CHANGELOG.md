...
- **fix(ci): install NetBSD build deps via direct `pkg_add` instead of pkgin (CI follow-up):** the NetBSD CI jobs (`netbsd-build-and-test`, `netbsd-pkg`, `netbsd-pkg-retry`) aborted at the prepare step with `ssh exited with code 127` because the pkgin bootstrap (`pkg_add -U pkgin`) fails on the current NetBSD 10.1 pkgsrc snapshot — its `pkg_install>=20200701` dependency is unavailable in the repo — so pkgin never installed and the subsequent `pkgin install ...` died with `pkgin: not found`. The build dependencies (clang, meson, ninja, libsodium, etc.) were therefore never installed. The prepare steps now install the package set directly with `pkg_add -U` (the same pattern the FreeBSD job already uses successfully), and the optional gdb install in the run step likewise uses `pkg_add` instead of pkgin. The jobs remain `continue-on-error` (non-blocking) but now reach the build step instead of dying in dependency setup.
