...
- **fix(ci): relax thumbnail worker `RLIMIT_CPU` under sanitizers to fix NetBSD/QEMU timeouts:** `src/media/thumbnail_worker_main.cpp` kept the production `RLIMIT_CPU` at 15 s but raises it to 120 s in sanitizer builds (ASan/UBSan/TSan). Instrumented workers under QEMU are too slow to decode, resample, and encode even a tiny PNG within the production CPU cap, so NetBSD timed out with HTTP 504. Production builds keep the tight 15-second limit.
