...
- **fix(platform): add glibc per-CPU syscalls to seccomp allowlist (CI follow-up):** the fork-based integration test (`test_seccomp_sqlite_flow`) revealed three additional syscalls that modern glibc (2.35+) calls under the filter: `rseq` (per-thread restartable-sequence registration re-done by the child after fork; also used in the `malloc` per-CPU cache on glibc 2.36+), `membarrier` (issued from the `malloc` fast path on SMP systems), and `getcpu` (per-CPU TLS cache). These were present on distros with glibc ≥ 2.36 (Debian, Fedora, OpenSUSE, RHEL) but not on Ubuntu 22.04 (glibc 2.35), explaining the distro-specific CI failures. `fallocate` is also added for SQLite's `posix_fallocate` pre-allocation path. The test child exit was changed from `_exit()` to a raw `exit_group` syscall to bypass ASan/LSan's `_exit` hook, which ran leak-checker cleanup code under the filter and triggered SIGSYS in sanitiser builds.
- **fix(ci): set `PKG_PATH` and install NetBSD build deps via direct `pkg_add` (CI follow-up):** the NetBSD CI jobs (`netbsd-build-and-test`, `netbsd-pkg`, `netbsd-pkg-retry`) never reached the build step. Root cause: NetBSD's `pkg_add` cannot resolve package stems without `PKG_PATH` pointing at a binary-package repository (unlike FreeBSD, which ships a default repo), and the `vmactions/netbsd-vm` image exports none — so every `pkg_add clang` failed with `no pkg found for 'clang'` and the build then aborted with `missing command: clang`/`meson`. The prior pkgin-based step was doubly broken: pkgin's own bootstrap (`pkg_add -U pkgin`) fails on the NetBSD 10.1 pkgsrc snapshot because its `pkg_install>=20200701` dependency is unavailable, so `pkgin install ...` died with exit 127 before building. The prepare steps now export `PKG_PATH` to the pkgsrc binary repository for the running OS release/arch and install the dependency set directly with `pkg_add -U` (`ftp.netbsd.org` origin rather than `cdn.netbsd.org`, whose Fastly backend was returning 503 "Backend is unhealthy"). The optional gdb install in the run step re-exports `PKG_PATH` (the run step is a separate shell) and uses `pkg_add`. The jobs remain `continue-on-error` (non-blocking) but now reach and complete the build step.
- **fix(test): seccomp+SQLite integration test drops to non-root to mirror production (CI follow-up):** the test still failed with `SIGSYS` on the Docker-based distro jobs (debian/fedora/opensuse/rhel) while ubuntu-native passed. A self-diagnosing variant — the production allowlist installed with `SECCOMP_RET_TRAP` plus a `SIGSYS` handler that reports each blocked syscall number through a pipe to the parent — identified the single missing syscall as `fchown` (nr 93). SQLite's `robustFchown` only issues `fchown` when `geteuid()==0` (sqlite3.c), and production never calls it: the homeserver runs as a non-root service user (privilege drop is external via systemd `User=`/OpenRC; there is no in-process `setuid`), so the database is always opened while non-root. The Docker CI containers ran the test as root, exercising a syscall set production never uses. The test child now drops to uid/gid 65534 (nobody) when running as root before opening the store, faithfully exercising the production non-root path. `fchown` is deliberately left OFF the allowlist — privilege-mutation calls remain blocked. The self-diagnosing `RET_TRAP`+`SIGSYS` harness is kept as a permanent regression guard: any future allowlist gap is named in the test output instead of an opaque `SIGSYS` death. Exposed `apply_seccomp_filter_with_default(default_action)` in the platform module; production `apply_seccomp_filter()` delegates to it with `SECCOMP_RET_KILL_PROCESS` (unchanged).
