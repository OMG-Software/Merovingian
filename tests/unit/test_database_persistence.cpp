    ...
    GIVEN("an empty schema state")
    {
        ...
        WHEN("the initial upgrade and explicit downgrade are applied")
        {
            ...
            THEN("the upgrade chain reaches the current schema version and is reversible")
            {
                ...
                REQUIRE(upgrade_plan.steps.size() == 10U);
                ...
                REQUIRE(upgraded.state.applied_migrations.size() == 10U);
                ...
                REQUIRE(downgrade_plan.steps.size() == 10U);
                // Downgrade walks v10->v0; the openid_tokens table drops
                // first, then notifications, then pushers, then the column
                // drop must precede the account_threepids table drop.
                REQUIRE(downgrade_plan.steps[0].name == "drop_openid_tokens");
                REQUIRE(downgrade_plan.steps[1].name == "drop_notifications");
                REQUIRE(downgrade_plan.steps[2].name == "drop_pushers");
                REQUIRE(downgrade_plan.steps[3].name == "drop_account_threepids_columns");
                REQUIRE(downgrade_plan.steps[4].name == "drop_account_threepids");
                REQUIRE(downgrade_plan.steps[5].name == "drop_backfill_state_transitions");
                REQUIRE(downgrade_plan.steps[6].name == "drop_state_transitions");
                REQUIRE(downgrade_plan.steps[7].name == "drop_event_stream_watermark");
                REQUIRE(downgrade_plan.steps[8].name == "drop_sync_stream_watermark");
                REQUIRE(downgrade_plan.steps[9].name == "drop_initial_schema");
    ...
    GIVEN("the initial schema inventory")
    {
        WHEN("core table membership is queried")
        {
            ...
            THEN("required Matrix storage areas have table-specific definitions")
            {
                ...
                // the notifications table, and migration v10 adds the
                // openid_tokens table.
                // The post-v1 tables (v2/v3/v4/v6/v8/v9/v10) are counted in
                // the current schema inventory, while v5 and v7 add no
                // tables.
                REQUIRE(tables.size() == 45U);
                REQUIRE(merovingian::database::current_schema_version() == 10U);
