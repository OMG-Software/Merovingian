    ...
    auto t1 = std::thread{[&]() {
        try
        {
            pair.server = std::make_unique<merovingian::ipc::IpcChannel>(
                std::move(server_fd), merovingian::ipc::IpcChannel::Role::server);
        }
        catch (...)
        {
    ...
    auto t2 = std::thread{[&]() {
        try
        {
            pair.client = std::make_unique<merovingian::ipc::IpcChannel>(
                std::move(client_fd), merovingian::ipc::IpcChannel::Role::client);
        }
        catch (...)
        {
...
SCENARIO("IpcChannel performs encrypted key exchange on construction",
...
SCENARIO("IpcChannel delivers a request and matching response",
         "[ipc][channel][request_response]")
{
    GIVEN("a connected server and client channel pair")
    {
        ...
        pair.server->set_request_handler(
            [&pair, &received, &received_id](std::uint64_t id, std::string json) {
                received_id = id;
                received    = json;
...
SCENARIO("IpcChannel delivers a notification without expecting a reply",
         "[ipc][channel][notification]")
{
    GIVEN("a connected server and client channel pair")
    {
        auto pair     = make_channel_pair();
        ...
        pair.server->set_request_handler(
            [&notified](std::uint64_t /*id*/, std::string /*json*/) { notified.store(true); });
        pair.server->start();
        pair.client->start();
...
SCENARIO("IpcChannel send_request returns nullopt on timeout when no reply is sent",
         "[ipc][channel][timeout]")
{
    GIVEN("a server that receives but never replies")
    {
        ...
        {
            auto const reply =
                pair.client->send_request(R"({"type":"wait"})", std::chrono::seconds{1});
