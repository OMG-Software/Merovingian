...
SCENARIO("HTTP request head parser accepts bounded simple requests", "[http][request]")
{
    GIVEN("a simple bounded HTTP request head")
    {
        ...
        WHEN("the request head is parsed")
        {
            ...
            THEN("the request fields are populated")
            {
...
SCENARIO("HTTP request head parser rejects malformed request lines", "[http][request]")
{
    GIVEN("a malformed HTTP request line")
    {
        ...
        WHEN("the request head is parsed")
        {
            ...
            THEN("a malformed request line error is returned")
            {
...
SCENARIO("HTTP request head parser rejects invalid methods and targets", "[http][request]")
{
    GIVEN("request heads with invalid method and target syntax")
    {
        ...
        WHEN("the request heads are parsed")
        {
            ...
            THEN("syntax errors are reported")
            {
...
SCENARIO("HTTP request head parser rejects duplicate mismatched content length", "[http][request]")
{
    GIVEN("a request head with mismatched duplicate content lengths")
    {
        ...
        WHEN("the request head is parsed")
        {
            ...
            THEN("duplicate content length is rejected")
            {
...
SCENARIO("HTTP request head parser rejects unsupported transfer encoding", "[http][request]")
{
    GIVEN("a request head using chunked transfer encoding")
    {
        ...
        WHEN("the request head is parsed")
        {
            ...
            THEN("unsupported transfer encoding is reported")
            {
