         ...
         "[homeserver][client-server][e2ee][otk-signature][regression]")
{
    ...
    {
        MEROVINGIAN_NETBSD_DIAG("otk-no-identity: start_client_server");
        auto started = merovingian::homeserver::start_client_server(registration_enabled_config());
        REQUIRE(started.started);
        MEROVINGIAN_NETBSD_DIAG("otk-no-identity: runtime started");
        auto& runtime = started.runtime;

        MEROVINGIAN_NETBSD_DIAG("otk-no-identity: register");
        REQUIRE(merovingian::homeserver::handle_client_server_request(
                    runtime, {"POST", "/_matrix/client/v3/register", {}, register_body("inviter", "CorrectHorse7!")})
                    .response.status == 200U);
        MEROVINGIAN_NETBSD_DIAG("otk-no-identity: registered");

        MEROVINGIAN_NETBSD_DIAG("otk-no-identity: login");
        auto const inviter_login = merovingian::homeserver::handle_client_server_request(
            runtime, {"POST",
                      "/_matrix/client/v3/login",
                      {},
        ...
        WHEN("the inviter uploads a signed_curve25519 one_time_key with no device_keys in the body or store")
        {
            ...
            MEROVINGIAN_NETBSD_DIAG("otk-no-identity: scenario end");
        }
    }
}
