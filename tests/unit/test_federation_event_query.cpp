...
[[nodiscard]] auto store_with_room_events() -> merovingian::database::PersistentStore
{
    ...
    store.events.push_back({"$create:remote.example.org", room_id, sender, R"({"type":"m.room.create"})", 1U, 1U,
                            {}, {}, {}});
    store.events.push_back({"$member:remote.example.org", room_id, sender, R"({"type":"m.room.member"})", 2U, 2U,
                            {}, {}, {}});
    store.events.push_back({"$message:remote.example.org", room_id, sender, R"({"type":"m.room.message"})", 5U, 3U,
                            {}, {}, {}});
...
SCENARIO("Federation event response returns a single signed PDU", "[federation][events][query]")
{
    GIVEN("a store containing the requested event")
    {
        ...
        {
            auto const body =
                merovingian::federation::build_event_response(store, "$nonexistent:remote.example.org",
                                                              "local.example.org");
...
SCENARIO("Federation state response returns the room's current state PDUs", "[federation][events][state]")
{
    GIVEN("a store with state events for a room")
    {
        ...
        {
            auto const body =
                merovingian::federation::build_state_response(store, "!room:remote.example.org");
...
SCENARIO("Federation state_ids response returns event IDs only", "[federation][events][state-ids]")
{
    GIVEN("a store with state events for a room")
    {
        ...
        {
            auto const body =
                merovingian::federation::build_state_ids_response(store, "!room:remote.example.org");
