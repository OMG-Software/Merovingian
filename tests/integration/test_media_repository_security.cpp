...
[[nodiscard]] auto register_and_login_admin(merovingian::homeserver::HomeserverRuntime& runtime) -> std::string
{
    auto const registration = merovingian::homeserver::handle_local_http_request(
        runtime,
        {"POST", "/_matrix/client/v3/register", {}, "alice|correct horse battery staple"}
    );
    REQUIRE(registration.status == 200U);

    auto const login = merovingian::homeserver::handle_local_http_request(
        runtime,
        {"POST", "/_matrix/client/v3/login", {}, "@alice:example.org|correct horse battery staple|DEVICE1"}
    );
...
SCENARIO("Integrated local media repository flow covers upload download dedupe quarantine release remove and metrics", "[media][repository][integration]")
{
    GIVEN("a running homeserver with media upload limits")
    {
        ...
        WHEN("local media is uploaded and administered through HTTP routes")
        {
            ...
            auto const quarantine = merovingian::homeserver::handle_local_http_request(
                runtime,
                {"POST", "/_merovingian/admin/media/quarantine/" + first_media_id, token, "policy review"}
            );
            ...
            auto const remove = merovingian::homeserver::handle_local_http_request(
                runtime,
                {"POST", "/_merovingian/admin/media/remove/" + first_media_id, token, "operator removal"}
            );
            ...
            auto const metrics = merovingian::homeserver::handle_local_http_request(
                runtime,
                {"GET", "/_merovingian/admin/media/metrics", token, {}}
            );
            ...
            THEN("media bytes are served only while available and metadata/audit paths are updated")
            {
                ...
                REQUIRE(blocked_download.status == 404U);
    ...
    GIVEN("a running homeserver with strict media policy")
    {
        ...
        WHEN("unsafe uploads are submitted")
        {
            auto const oversized = merovingian::homeserver::handle_local_http_request(
                runtime,
                {"POST", "/_matrix/media/v3/upload", token, "text/plain|text/plain|clean|too-large"}
            );
            ...
            THEN("oversized and unknown MIME uploads are rejected and scanner failures are quarantined")
            {
                REQUIRE(oversized.status == 400U);
                REQUIRE(oversized.body == "media upload exceeds size limit");
                REQUIRE(unknown_mime.status == 400U);
                REQUIRE(unknown_mime.body == "media MIME type is not allowed");
                REQUIRE(scanner_failure.status == 200U);
...
SCENARIO("Remote media download route fails closed until a later milestone enables fetching", "[media][repository][integration]")
{
    GIVEN("a running homeserver with remote media disabled")
    {
        ...
        WHEN("a remote media download is requested")
        {
            auto const remote = merovingian::homeserver::handle_local_http_request(
                runtime,
                {"GET", "/_matrix/media/v3/download/remote.example.org/media123", {}, {}}
            );
...
SCENARIO("Integrated media repository policy still rejects unsafe remote boundary inputs", "[media][security][integration]")
{
