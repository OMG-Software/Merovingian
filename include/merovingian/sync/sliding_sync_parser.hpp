...
[[nodiscard]] auto parse_sliding_sync_request(std::string_view body)
...
[[nodiscard]] auto parse_sliding_sync_pos(std::string_view target)
    -> std::optional<StreamToken>;

// Extract ?timeout= from a request target. Returns nullopt when absent or
// not a non-negative integer.
[[nodiscard]] auto parse_sliding_sync_timeout(std::string_view target)
...
[[nodiscard]] auto sliding_sync_ranges_valid(std::vector<SlidingSyncRange> const& ranges) noexcept
    -> bool;
