...
[[nodiscard]] auto client_auth_database_statements(ClientAuthEndpoint endpoint, std::string_view user_id,
                                                   std::string_view device_id, TokenHash const& token_hash)
    -> std::vector<database::PreparedStatement>;
[[nodiscard]] auto make_client_auth_boundary_plan(ClientAuthRoute const& route, std::string_view user_id,
                                                  std::string_view device_id, TokenHash const& token_hash, bool allowed,
                                                  std::string_view reason) -> ClientAuthBoundaryPlan;
