...
[[nodiscard]] inline auto trim_ascii(std::string_view value) noexcept -> std::string_view
{
...
[[nodiscard]] inline auto parse_bool_value(std::string_view value, bool& output) noexcept -> bool
{
    if (value == "true")
    {
        output = true;
...
[[nodiscard]] inline auto parse_u32_value(std::string_view value, std::uint32_t& output) noexcept -> bool
{
    if (value.empty())
    {
        return false;
...
[[nodiscard]] inline auto parse_string_list(std::string_view value) -> std::vector<std::string>
{
...
inline auto apply_config_value(
    ServerConfig& server,
...
[[nodiscard]] inline auto parse_key_value_config(std::string_view input) -> ConfigParseResult
{
