...
grammar[grammar](/v1.19/appendices/#server-name). Where applicable, a delegated
...
If the hostname is an IP literal, then that IP address should be
...
If the hostname is not an IP literal, a regular HTTPS request is
made to `https://<hostname>/.well-known/matrix/server` (according to
RFC 8615[RFC 8615](https://datatracker.ietf.org/doc/html/rfc8615)), expecting
...
present a valid certificate for `<delegated_hostname>`.
  ...
  3. **[Added in `v1.8`]** If `<delegated_hostname>` is not an IP literal and no
...
**[Added in `v1.8`]** If the `/.well-known` request resulted in an error response, a server is
...
If the `/.well-known` request returned an error response, and
...
2. Consistency with the recommendations in RFC6125[RFC6125](https://datatracker.ietf.org/doc/html/rfc6125#section-6.2.1)
and other applications using SRV records such XMPP[XMPP](https://datatracker.ietf.org/doc/html/rfc6120#section-13.7.2.1).


Note that the target of a SRV record may *not* be a CNAME, as
mandated by RFC2782[RFC2782](https://www.rfc-editor.org/rfc/rfc2782.html):
...
The IANA registration for port 8448 and `matrix-fed` can be found here[here](https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml?search=matrix-fed).
...
server name[server name](/v1.19/appendices/#server-name) by making a
GET request to `https://hostname/.well-known/matrix/server`.| Rate-limited: | No |
| --- | --- |
...
| --- | --- | --- |
| `m.server` | `string` | The server name to delegate server-server communications to, with optional port. The delegated server name uses the same grammar as server names in the appendices[server names in the appendices](/v1.19/appendices/#server-name). |
...
Get the implementation name and version of this homeserver.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
Server Keys
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `old_verify_keys` | `{string: Old Verify KeyOld Verify Key}` | The public keys that the server used to use and when it stopped using them. The object’s key is the algorithm and version combined (`ed25519` being the algorithm and `0ldK3y` being the version in the example below). Together, this forms the Key ID. The version must have characters matching the regular expression `[a-zA-Z0-9_]`. |
| `server_name` | `string` | **Required: **The homeserver’s server name[server name](/v1.19/appendices/#server-name). |
| `signatures` | `{string: {string: string}}` | **Required: **Digital signatures for this object signed using the `verify_keys`. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). |
| `valid_until_ts` | `integer` | **Required: **POSIX timestamp in milliseconds when the list of valid keys should be refreshed. This field MUST be ignored in room versions 1, 2, 3, and 4. Keys used beyond this timestamp MUST be considered invalid, depending on the room version specification[room version specification](/v1.19/rooms/). Servers MUST use the lesser of this field and 7 days into the future when determining if a key is valid. This is to avoid a situation where an attacker publishes a key which is valid for a significant amount of time without a way for the homeserver owner to revoke it. |
| `verify_keys` | `{string: Verify KeyVerify Key}` | **Required: **Public keys of the homeserver for verifying digital signatures. The object’s key is the algorithm and version combined (`ed25519` being the algorithm and `abc123` being the version in the example below). Together, this forms the Key ID. The version must have characters matching the regular expression `[a-zA-Z0-9_]`. |
...
| `key` | `string` | **Required: **The Unpadded base64[Unpadded base64](/v1.19/appendices/#unpadded-base64) encoded key. |
...
server must sign the keys returned by the queried servers.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
Query Criteria
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `minimum_valid_until_ts` | `integer` | A millisecond POSIX timestamp. The returned keys SHOULD be valid until at least this timestamp. If not supplied, the notary server MUST use the current time. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `server_keys` | `[Server KeysServer Keys]` | The queried server’s keys, signed by the notary server. |
...
sign the keys returned by the queried server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
path parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `serverName` | `string` | **Required: **The server name[server name](/v1.19/appendices/#server-name) to query |
...
algorithm[JSON signing
algorithm](/v1.19/appendices/#signing-json). The resulting signatures are added
...
section[server name resolution
section](/v1.19/server-server-api/#resolving-server-names) above do not affect these - the server
...
Section 11.4 of RFC 9110[Section 11.4 of RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110#section-11.4). In
...
Section 5.6.2 of RFC 9110[Section 5.6.2 of RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110#section-5.6.2); if a
...
- `destination`: **[Added in `v1.3`]** the server name of the receiving
...
receiving server must deny the request with an HTTP status code `401 Unauthorized`.
...
**[Changed in `v1.11`]**
This section used to reference RFC 7235[RFC 7235](https://datatracker.ietf.org/doc/html/rfc7235#section-2.1)
and RFC 7230[RFC 7230](https://datatracker.ietf.org/doc/html/rfc9110#section-5.6.2), that
...
the room version specification[room version specification](/v1.19/rooms/) for precise event formats.| Rate-limited: | No |
| --- | --- |
...
Transaction
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `edus` | `[Ephemeral Data UnitEphemeral Data Unit]` | List of ephemeral messages. May be omitted if there are no ephemeral messages to be sent. Must not include more than 100 EDUs. |
...
| `pdus` | `[PDU]` | **Required: **List of persistent updates to rooms. Must not include more than 50 PDUs. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
PDU Processing Results
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `pdus` | `{Event ID[Event ID](/v1.19/appendices#event-ids): PDU Processing ResultPDU Processing Result}` | **Required: **The PDUs from the original transaction. The string key represents the ID of the PDU (event) that was processed. |
...
specification[room version
specification](/v1.19/rooms/).
...
1. **[Changed in `v1.16`]** Is a valid event, otherwise it is dropped. For
an event to be valid, it must comply with the event format of that room version[room version](/v1.19/rooms/).
...
section of the room version specifications[room version specifications](/v1.19/rooms/) for details on when it
is required.
...
7. **[Added in `v1.18`]** Is validated[validated](/v1.19/server-server-api/#validating-policy-server-signatures)
by the Policy Server, if the room is using a Policy Server[using a Policy Server](/v1.19/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room),
otherwise it is “soft failed”.
...
The Signing Events[Signing Events](/v1.19/server-server-api/#signing-events) section has more information on
...
the current `m.room.power_levels`[m.room.power_levels](/v1.19/client-server-api/#mroompower_levels)
...
For an `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember) state event, the
user given by the `state_key` of the event.

Some room versions[room versions](/v1.19/rooms/) accept power level values to be represented as
...
See the room version specification[room version specification](/v1.19/rooms/) for more information.
...
**[Changed in `v1.16`]** Depending on the room version[room version](/v1.19/rooms/), the
`m.room.create` event.
...
is present, and the room version supports restricted rooms[room version supports restricted rooms](/v1.19/rooms/#feature-matrix),
...
**[Added in `v1.18`]**
Events can also be soft failed if they fail Policy Server checks[Policy Server checks](/v1.19/server-server-api/#validating-policy-server-signatures).
...
Retrieves the complete auth chain for a given event.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `auth_chain` | `[PDU]` | **Required: **The PDUs[PDUs](/v1.19/server-server-api/#pdus) forming the auth chain of the given event. The event format varies depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
the room version specification[room version specification](/v1.19/rooms/).
...
the PDUs that preceded them are retrieved, up to the total number given by the `limit`.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `origin` | `string` | **Required: **The `server_name` of the homeserver sending this transaction. |
| `origin_server_ts` | `integer` | **Required: **POSIX timestamp in milliseconds on originating homeserver when this transaction started. |
| `pdus` | `[PDU]` | **Required: **List of persistent updates to rooms. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
and stopping at the `limit`.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `events` | `[PDU]` | **Required: **The missing events. The event format varies depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
Retrieves a single event.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `origin` | `string` | **Required: **The `server_name` of the homeserver sending this transaction. |
| `origin_server_ts` | `integer` | **Required: **POSIX timestamp in milliseconds on originating homeserver when this transaction started. |
| `pdus` | `[PDU]` | **Required: **A single PDU. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
Retrieves a snapshot of a room’s state at a given event.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
## Responses| Status | Description |
| --- | --- |
| `200` | The fully resolved state for the room, prior to considering any state changes induced by the requested event. Includes the authorisation chain for the events. |


### 200 response| Name | Type | Description |
| --- | --- | --- |
| `auth_chain` | `[PDU]` | **Required: **The full set of authorisation events that make up the state of the room, and their authorisation events, recursively. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
| `pdus` | `[PDU]` | **Required: **The fully resolved state of the room at the given event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
however this returns just the event IDs rather than the full events.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `auth_chain_ids` | `[string]` | **Required: **The full set of authorisation events that make up the state of the room, and their authorisation events, recursively. |
...
client-server endpoint[client-server endpoint](/v1.19/client-server-api/#get_matrixclientv1roomsroomidtimestamp_to_event)
...
`/event/{event_id}`[/event/{event_id}](/v1.19/server-server-api/#get_matrixfederationv1eventeventid)
endpoint so that it is available to for a client to query.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
`/query/directory`[/query/directory](/v1.19/server-server-api/#get_matrixfederationv1querydirectory) API endpoint. In the case of a new user joining a
...
discarded. The caller MAY try to join through another server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `userId` | `string` | **Required: **The user ID the join event will be for. This MUST be a user ID on the origin server. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used for the rest of the Joining Rooms[Joining Rooms](/v1.19/server-server-api/#joining-rooms) handshake. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
| `400` | The request is invalid, the room the server is attempting to join has a version that is not listed in the `ver` parameters, or the server was unable to validate restricted room conditions[restricted room conditions](/v1.19/server-server-api/#restricted-rooms). The error should be passed through to clients so that they may give better feedback to users. New in `v1.2`, the following error conditions might happen: If the room is restricted[restricted](/v1.19/client-server-api/#restricted-rooms) and none of the conditions can be validated by the server then the `errcode` `M_UNABLE_TO_AUTHORISE_JOIN` must be used. This can happen if the server does not know about any of the rooms listed as conditions, for example. `M_UNABLE_TO_GRANT_JOIN` is returned to denote that a different server should be attempted for the join. This is typically because the resident server can see that the joining user satisfies one or more conditions, such as in the case of restricted rooms[restricted rooms](/v1.19/client-server-api/#restricted-rooms), but the resident server would be unable to meet the auth rules governing `join_authorised_via_users_server` on the resulting `m.room.member` event. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `event` | `Event TemplateEvent Template` | An unsigned template event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
Event Template
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Membership Event ContentMembership Event Content` | **Required: **The content of the event. |
...
the room version specification[room version specification](/v1.19/rooms/) for precise event formats.
...
- The join event fails a signature check[signature check](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
client-server API[client-server API](/v1.19/client-server-api/#restricted-rooms) and are available
in room versions which support restricted join rules[which support restricted join rules](/v1.19/rooms/#feature-matrix).
...
much like joining rooms[joining rooms](/v1.19/server-server-api/#joining-rooms) the server
...
In case any of the above checks fail, the response MUST be treated as malformed and
discarded. The caller MAY try to knock through another server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `userId` | `string` | **Required: **The user ID the knock event will be for. This MUST be a user ID on the origin server. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used for the rest of the Knocking Rooms[Knocking Rooms](/v1.19/server-server-api/#knocking-rooms) handshake. Note that events have a different format depending on room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `event` | `Event TemplateEvent Template` | **Required: **An unsigned template event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
- The knock event fails a signature check[signature check](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
| `400` | The request is invalid in some way. |
| `403` | The knocking server or user is not permitted to knock on the room, such as when the server/user is banned or the room is not set up for receiving knocks. |
| `404` | The room that the knocking server is attempting to knock upon is unknown to the receiving server. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `knock_room_state` | `[PDU]` | **Required: **A list of state events to help the initiator of the knock identify the room. Translated as stripped state events[stripped state events](/v1.19/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Entries which are improperly signed[improperly signed](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events) or formatted SHOULD be removed by the server prior to supplying them over the Client-Server API. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
**[Added in `v1.16`]** `invite_room_state` MUST now have its entries formatted
according to the room’s version (see room version specification[room version specification](/v1.19/rooms/)). However,
...
room version specification[room version specification](/v1.19/rooms/) for precise event formats. **The request and response
...
transaction[federation
transaction](/v1.19/server-server-api/#transactions).
...
- The invite event fails a signature check[signature check](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
- One or more events fails a signature check[signature check](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
InviteEvent
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Membership Event ContentMembership Event Content` | **Required: **The content of the event, matching what is available in the Client-Server API[Client-Server API](/v1.19/client-server-api/). Must include a `membership` of `invite`. |
...
| `type` | `string` | **Required: **The value `m.room.member`. |
| `unsigned` | `UnsignedDataUnsignedData` | Information included alongside the event that is not signed. May include more than what is listed here. |
...
UnsignedData
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `invite_room_state` | `[PDU]` | A list of state events to help the receiver of the invite identify the room. Translated as stripped state events[stripped state events](/v1.19/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Servers might need to apply validation to the `invite_room_state` depending on room version. See endpoint description for more information. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | The event with the invited server’s signature added. All other fields of the events should remain untouched. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
| `400` | The request is invalid in some way. |
| `403` | The invite is not allowed. The `M_FORBIDDEN` error code is used to indicate one of the following: - The sender is not allowed to send invites to the target user/homeserver. - The homeserver does not permit anyone to invite its users. - The homeserver refuses to participate in the room. **[Added in `v1.18`]** The `M_INVITE_BLOCKED` error code is used to indicate that the homeserver rejected the invite due to invite blocking[invite blocking](/v1.19/client-server-api/#invite-permission). |
...
Event Container
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `event` | `InviteEventInviteEvent` | **Required: **An invite event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
...
| --- | --- | --- |
| `event` | `InviteEventInviteEvent` | **Required: **An invite event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. |
| `invite_room_state` | `[PDU]` | A list of state events to help the receiver of the invite identify the room. Translated as stripped state events[stripped state events](/v1.19/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Servers might need to apply validation to the `invite_room_state` depending on room version. See the endpoint description for more information. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
Similar to the Joining Rooms[Joining Rooms](/v1.19/server-server-api/#joining-rooms) handshake, the server
...
In case any of the above checks fail, the response MUST be treated as malformed and
discarded. The caller MAY try to leave through another server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `userId` | `string` | **Required: **The user ID the leave event will be for. This MUST be a user ID on the origin server. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used to call `/send_leave`. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.19/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
| `403` | The request is not authorised. This could mean that the user is not in the room. |
...
- The leave event fails a signature check[signature check](/v1.19/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
under the Third-party invites[Third-party invites](/v1.19/client-server-api/#third-party-invites) module.
...
API](/v1.19/identity-service-api/).
...
request[lookup
request](/v1.19/identity-service-api/#post_matrixidentityv2lookup) on the identity
server will return it. The invite is then processed by the inviting homeserver
as a standard `m.room.member` invite event[standard m.room.member invite event](/v1.19/server-server-api/#inviting-to-a-room). This is the
simplest case.
...
homeserver will request the identity server to store an invite[store an invite](/v1.19/identity-service-api/#invitation-storage)
for this identifier and to deliver it to whoever binds it to its Matrix ID. It
will also send an `m.room.third_party_invite`[m.room.third_party_invite](/v1.19/client-server-api/#mroomthird_party_invite)
...
the identity server will send a request to the `/3pid/onbind`[/3pid/onbind](/v1.19/server-server-api/#put_matrixfederationv13pidonbind)
endpoint of the the ID’s homeserver as described in the Invitation
Storage[Invitation
Storage](/v1.19/identity-service-api/#invitation-storage) section of the Identity
Service API.

The following process applies for each invite sent by the identity
server:

The invited homeserver will create an `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember)
...
at the `/exchange_third_party_invite`[/exchange_third_party_invite](/v1.19/server-server-api/#put_matrixfederationv1exchange_third_party_inviteroomid)
endpoint.
...
room invites the identity server has been made aware of.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
| `invites` | `[Third-party InviteThird-party Invite]` | **Required: **A list of pending invites that the third-party identifier has received. |
| `medium` | `string` | **Required: **The type of third-party identifier. Currently only “email” is a possible value. |
| `mxid` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The user that is now bound to the third-party identifier. |
...
| `mxid` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The now-bound user ID that received the invite. |
| `room_id` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The room ID the invite is valid for. |
| `sender` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The user ID that sent the invite. |
| `signed` | `SignedThirdPartyInviteSignedThirdPartyInvite` | **Required: **A block of content which has been signed by the identity server, which homeservers can use to verify the event. Clients should ignore this. |
...
SignedThirdPartyInvite
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `mxid` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The user ID that has been bound to the third-party identifier. |
| `signatures` | `{Server Name[Server Name](/v1.19/appendices#server-name): {string: Unpadded Base64[Unpadded Base64](/v1.19/appendices#unpadded-base64)}}` | **Required: **The identity server signatures for this block. This is a map of identity server name to signing key identifier to base64-encoded signature. The signatures are calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). |
| `token` | `string` | **Required: **The token generated by the identity server at the `/store_invite`[/store_invite](/v1.19/identity-service-api/#post_matrixidentityv2store-invite) endpoint. It matches the `state_key` of the corresponding `m.room.third_party_invite`[m.room.third_party_invite](/v1.19/client-server-api/#mroomthird_party_invite) event. |
...
an invite as per the Inviting to a room[Inviting to a room](/v1.19/server-server-api/#inviting-to-a-room) section before returning a
response to this request.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
path parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `roomId` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The room ID to exchange a third-party invite in |


### Request body| Name | Type | Description |
| --- | --- | --- |
| `content` | `Event ContentEvent Content` | **Required: **The event content |
| `room_id` | `string` | **Required: **The room ID the event is for. Must match the ID given in the path. format: mx-room-id pattern: “^!” |
| `sender` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The user ID of the user who sent the original `m.room.third_party_invite` event. |
| `state_key` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The user ID of the invited user |
...
| `third_party_invite` | `Third-party InviteThird-party Invite` | **Required: **The third-party invite |
...
To complement the room directory in the Client-Server API[room directory in the Client-Server API](/v1.19/client-server-api/#published-room-directory),
...
just those listed on the receiving homeserver’s directory.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `third_party_instance_id` | `string` | The specific third-party network/protocol to request from the homeserver. Can only be used if `include_all_networks` is false. This is the `instance_id` of a `Protocol Instance` returned by `GET /_matrix/client/v3/thirdparty/protocols`[GET /_matrix/client/v3/thirdparty/protocols](/v1.19/client-server-api/#get_matrixclientv3thirdpartyprotocols). |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `chunk` | `[PublishedRoomsChunkPublishedRoomsChunk]` | **Required: **A paginated chunk of published rooms. |
...
PublishedRoomsChunk
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `avatar_url` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | The URL for the room’s avatar, if one is set. |
| `canonical_alias` | `Room Alias[Room Alias](/v1.19/appendices#room-aliases)` | The canonical alias of the room, if any. |
...
| `room_id` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The ID of the room. |
| `room_type` | `string` | The `type` of room (from `m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate)), if any. |
| `topic` | `string` | The plain text topic of the room. Omitted if no `text/plain` mimetype exists in `m.room.topic`[m.room.topic](/v1.19/client-server-api/#mroomtopic). |
| `world_readable` | `boolean` | **Required: **Whether the room may be viewed by users without joining. |
...
returned by this endpoint. Previously, rooms were ordered with the largest
joined member count first. Continuing to order by largest first or another
stable order is recommended.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `filter` | `FilterFilter` | Filter to apply to the results. |
...
| `room_types` | `[string\|null]` | An optional list of room types[room types](/v1.19/client-server-api/#types) to search for. To include rooms without a room type, specify `null` within this list. When not specified, all applicable rooms (regardless of type) are returned. **Added in `v1.4`** |
...
To complement the Client-Server API’s Spaces module[Client-Server API’s Spaces module](/v1.19/client-server-api/#spaces),
...
Federation version of the Client-Server `GET /hierarchy`[GET /hierarchy](/v1.19/client-server-api/#get_matrixclientv1roomsroomidhierarchy)
...
Only `m.space.child`[m.space.child](/v1.19/client-server-api/#mspacechild) state events of the room are considered. Invalid child
rooms and parent events are not covered by this endpoint.

Responses to this endpoint should be cached for a period of time.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `suggested_only` | `boolean` | Optional (default `false`) flag to indicate whether or not the server should only consider suggested rooms. Suggested rooms are annotated in their `m.space.child`[m.space.child](/v1.19/client-server-api/#mspacechild) event contents. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `children` | `[SpaceHierarchyChildRoomsChunkSpaceHierarchyChildRoomsChunk]` | **Required: **A summary of the space’s children. Rooms which the requesting server cannot peek/join will be excluded. |
...
| `room` | `SpaceHierarchyParentRoomSpaceHierarchyParentRoom` | **Required: **A summary of the room requested. |
...
SpaceHierarchyChildRoomsChunk
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `allowed_room_ids` | `[Room ID[Room ID](/v1.19/appendices#room-ids)]` | If the room is a restricted room[restricted room](/v1.19/server-server-api/#restricted-rooms), these are the room IDs which are specified by the join rules. Empty or omitted otherwise. |
| `avatar_url` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | The URL for the room’s avatar, if one is set. |
| `canonical_alias` | `Room Alias[Room Alias](/v1.19/appendices#room-aliases)` | The canonical alias of the room, if any. |
...
| `num_joined_members` | `integer` | **Required: **The number of members joined to the room. |
| `room_id` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The ID of the room. |
| `room_type` | `string` | The `type` of room (from `m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate)), if any. **Added in `v1.4`** |
...
| `avatar_url` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | The URL for the room’s avatar, if one is set. |
| `canonical_alias` | `Room Alias[Room Alias](/v1.19/appendices#room-aliases)` | The canonical alias of the room, if any. |
| `children_state` | `[StrippedStateEventStrippedStateEvent]` | **Required: **The `m.space.child`[m.space.child](/v1.19/client-server-api/#mspacechild) events of the space-room, represented as Stripped State Events[Stripped State Events](/v1.19/client-server-api/#stripped-state) with an added `origin_server_ts` key. If the room is not a space-room, this should be empty. |
| `encryption` | `string` | The encryption algorithm to be used to encrypt messages sent in the room. One of: `[m.megolm.v1.aes-sha2]`. **Added in `v1.15`** |
| `guest_can_join` | `boolean` | **Required: **Whether guest users may join the room and participate in it. If they can, they will be subject to ordinary power level rules like any other user. |
| `join_rule` | `string` | The room’s join rule. When not present, the room is assumed to be `public`. |
| `name` | `string` | The name of the room, if any. |
...
| `sender` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The `sender` for the event. |
...
m.typing
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Typing NotificationTyping Notification` | **Required: **The typing notification. |
...
m.presence
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Presence UpdatePresence Update` | **Required: **The presence updates and requests. |
...
Presence Update
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `push` | `[User Presence UpdateUser Presence Update]` | **Required: **A list of presence updates that the receiving server is likely to be interested in. |
...
m.receipt
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `{Room ID[Room ID](/v1.19/appendices#room-ids): Room ReceiptsRoom Receipts}` | **Required: **Receipts for a particular room. The string key is the room ID for which the receipts under it belong. |
...
Room Receipts
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `m.read` | `{User ID[User ID](/v1.19/appendices#user-identifiers): User Read ReceiptUser Read Receipt}` | **Required: **Read receipts for users in the room. The string key is the user ID the receipt belongs to. |
...
User Read Receipt
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `data` | `Read Receipt MetadataRead Receipt Metadata` | **Required: **Metadata for the read receipt. |
...
that belong to the target server (identified by the server name[server name](/v1.19/appendices/#server-name)
in the alias).

Servers may wish to cache the response to this query to avoid requesting the
information too often.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
to the target server (identified by the server name[server name](/v1.19/appendices/#server-name)
in the user ID).
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `field` | `string` | The field of the profile to query. If specified, the server MUST only return the given field in the response. If not specified, the server MUST return the full, public, profile for the user. Defined values are `displayname`, `avatar_url` and `m.tz`. In addition to these servers MAY allow users to set additional key/value pairs. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | The profile for the user. If a `field` is specified in the request, the response MUST only include the specified `field`. If no `field` was specified, the response SHOULD include all of the fields of the user’s profile that can be made public, such as the display name and avatar. If a field in the user’s profile can’t be made public over the federation, the responding server MAY choose to exclude it in the response. If the user does not have a particular field set on their profile, the server MUST either exclude it from the response body or give it the value `null`. |
| `403` | The server is unwilling to disclose whether the user exists and/or has profile information. |
| `404` | The user does not exist, does not have a profile or the queried field does not exist. |


### 200 response| Name | Type | Description |
| --- | --- | --- |
| `avatar_url` | `string` | The avatar URL for the user’s avatar. MUST either be omitted or set to `null` if the user does not have an avatar set. |
| `displayname` | `string` | The display name of the user. MUST either be omitted or set to `null` if the user does not have a display name set. |
| `m.tz` | `string` | The name of the user’s time zone. The name SHOULD be registered in the IANA Time Zone Database[IANA Time Zone Database](https://www.iana.org/time-zones). Requesting servers MUST tolerate invalid or unknown values. **Added in `v1.16`** |
| <Other properties> | `` | Additional profile fields. |
...
are specified as their own endpoints as an extension to this definition.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
User ID of the owner.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `sub` | `string` | **Required: **The Matrix User ID who generated the token. The caller MUST validate that the returned user ID is on the server they called (i.e. if you make a request to example.com and it returns `@alice:matrix.org`, the result is invalid). |
...
module](/v1.19/client-server-api/#device-management)
...
Gets information on all of the user’s devices| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `devices` | `[User DeviceUser Device]` | **Required: **The user’s devices. May be empty. |
| `master_key` | `CrossSigningKeyCrossSigningKey` | The user's master signing key. |
| `self_signing_key` | `CrossSigningKeyCrossSigningKey` | The user's self-signing key. |
...
| `keys` | `DeviceKeysDeviceKeys` | **Required: **Identity keys for the device. |
...
| `signatures` | `{User ID[User ID](/v1.19/appendices#user-identifiers): {string: string}}` | **Required: **Signatures for the device key object. A map from user ID, to a map from `<algorithm>:<device_id>` to the signature. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). |
...
| `signatures` | `Signatures` | Signatures of the key, calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). Optional for the master signing key. Other keys must be signed by the user's master signing key. |
...
m.device_list_update
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Device List UpdateDevice List Update` | **Required: **The description of the device whose details has changed. |
...
| `keys` | `DeviceKeysDeviceKeys` | The updated identity keys (if any) for this device. May be absent if the device has no E2E keys defined. |
...
module[End-to-End Encryption
module](/v1.19/client-server-api/#end-to-end-encryption)
...
/keys/upload[/keys/upload](/v1.19/client-server-api/#post_matrixclientv3keysupload). (All
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `one_time_keys` | `{User ID[User ID](/v1.19/appendices#user-identifiers): {string: string}}` | **Required: **The keys to be claimed. A map from user ID, to a map from device ID to algorithm name. Requested users must be local to the receiving homeserver. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `one_time_keys` | `{User ID[User ID](/v1.19/appendices#user-identifiers): {string: {string: string\|KeyObjectKeyObject}}}` | **Required: **One-time keys for the queried devices. A map from user ID, to a map from devices to a map from `<algorithm>:<key_id>` to the key object. See the Client-Server Key Algorithms[Client-Server Key Algorithms](/v1.19/client-server-api/#key-algorithms) section for more information on the Key Object format. |
...
| `signatures` | `{string: {string: string}}` | **Required: **Signature of the key object. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). |
...
Returns the current devices and identity keys for the given users.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `device_keys` | `{User ID[User ID](/v1.19/appendices#user-identifiers): [string]}` | **Required: **The keys to be downloaded. A map from user ID, to a list of device IDs, or to an empty list to indicate all devices for the corresponding user. Requested users must be local to the receiving homeserver. |
...
| `keys` | `{string: string}` | **Required: **Public identity keys. The names of the properties should be in the format `<algorithm>:<device_id>`. The keys themselves should be encoded as specified by the key algorithm. |
| `signatures` | `{User ID[User ID](/v1.19/appendices#user-identifiers): {string: string}}` | **Required: **Signatures for the device key object. A map from user ID, to a map from `<algorithm>:<device_id>` to the signature. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.19/appendices/#signing-json). |
| `unsigned` | `UnsignedDeviceInfoUnsignedDeviceInfo` | Additional data added to the device key information by intermediate servers, and not covered by the signatures. |
| `user_id` | `string` | **Required: **The ID of the user the device belongs to. Must match the user ID used when logging in. |
...
m.signing_key_update
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Signing Key UpdateSigning Key Update` | **Required: **The updated signing keys. |
| `edu_type` | `string` | **Required: **The string `m.signing_key_update`. One of: `[m.signing_key_update]`. |
...
Signing Key Update
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `master_key` | `CrossSigningKeyCrossSigningKey` | Key used for cross signing |
| `self_signing_key` | `CrossSigningKeyCrossSigningKey` | Key used for cross signing |
...
m.direct_to_device
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `To Device MessageTo Device Message` | **Required: **The description of the direct-to-device message. |
...
| `messages` | `{User ID[User ID](/v1.19/appendices#user-identifiers): {string: Device Message Contents}}` | **Required: **The contents of the messages to be sent. These are arranged in a map of user IDs to a map of device IDs to message bodies. The device ID may also be `*`, meaning all known devices for the user. |
...
API[Client-Server
API](/v1.19/client-server-api/#content-repository). When a server wishes
to serve content originating from a remote server, it needs to ask the
remote server for the media.

Servers MUST use the server described in the Matrix Content URI[Matrix Content URI](/v1.19/client-server-api/#matrix-content-mxc-uris).
Formatted as `mxc://{ServerName}/{MediaID}`, servers MUST download the media from
`ServerName` using the below endpoints.

**[Changed in `v1.11`]** Servers were previously advised to use the `/_matrix/media/*`
endpoints described by the Content Repository module in the Client-Server API[Content Repository module in the Client-Server API](/v1.19/client-server-api/#content-repository),
...
**Added in `v1.11`**| Rate-limited: | Yes |
| --- | --- |
| Requires authentication: | Yes |
...
| `504` | The content is not yet available. A standard error response[standard error response](/v1.19/client-server-api/#standard-error-response) will be returned with the `errcode` `M_NOT_YET_UPLOADED`. |
...
See the Client-Server API Thumbnails[Client-Server API Thumbnails](/v1.19/client-server-api/#thumbnails)
section for more information.| Rate-limited: | Yes |
| --- | --- |
| Requires authentication: | Yes |
...
| `method` | `string` | The desired resizing method. See the Client-Server API Thumbnails[Client-Server API Thumbnails](/v1.19/client-server-api/#thumbnails) section for more information. One of: `[crop, scale]`. |
| `timeout_ms` | `integer` | The maximum number of milliseconds that the client is willing to wait to start receiving data, in the case that the content has not yet been uploaded. The default value is 20000 (20 seconds). The content repository SHOULD impose a maximum value for this parameter. The content repository MAY respond before the timeout. **Added in `v1.7`** |
...
ACLs[Server
ACLs](/v1.19/client-server-api/#server-access-control-lists-acls-for-rooms)
...
Additionally the `/_matrix/federation/v1/send/{txnId}`[/_matrix/federation/v1/send/{txnId}](/v1.19/server-server-api/#put_matrixfederationv1sendtxnid)
...
ACLs MUST be applied to all EDUs that are local to a specific room:
  ...
  - For typing notifications (`m.typing`)[typing notifications (m.typing)](/v1.19/server-server-api/#typing-notifications), if
the sending server is denied access to the room identified by
`room_id`, the EDU MUST be ignored.

  - For receipts (`m.receipt`)[receipts (m.receipt)](/v1.19/server-server-api/#receipts), all receipts for a particular
...
- `/_matrix/policy/v1/sign`[/_matrix/policy/v1/sign](/v1.19/server-server-api/#post_matrixpolicyv1sign) - **[Added in `v1.18`]**
...
- Normal server name resolution[Normal server name resolution](/v1.19/server-server-api/#resolving-server-names).

- Publishing a signing key[Publishing a signing key](/v1.19/server-server-api/#publishing-keys).

- Request authentication[Request authentication](/v1.19/server-server-api/#authentication).

- Being able to make and send join requests[make and send join requests](/v1.19/server-server-api/#joining-rooms).

- Receiving and processing `POST /_matrix/policy/v1/sign`[POST /_matrix/policy/v1/sign](/v1.19/server-server-api/#post_matrixpolicyv1sign)
requests.
...
- Receiving invites[invites](/v1.19/server-server-api/#inviting-to-a-room) can make it easier to add the Policy
Server to a room.

- Receiving transactions[transactions](/v1.19/server-server-api/#transactions) is recommended to avoid remote servers
flagging the Policy Server as “offline”, even if the contents are discarded.

- Receiving device lookups[device lookups](/v1.19/server-server-api/#get_matrixfederationv1userdevicesuserid) can also
...
- The *current state* for the room has a valid `m.room.policy`[m.room.policy](/v1.19/client-server-api/#mroompolicy)
...
Policy Server[ask that
Policy Server](/v1.19/server-server-api/#asking-for-a-policy-server-signature-on-an-event) for a signature
before sending an event. If the Policy Server refuses to sign the event being
sent, servers SHOULD fail to send that event as per the validation rules[validation rules](/v1.19/server-server-api/#validating-policy-server-signatures).

“Current state” shares the same definition as soft failure[soft failure](/v1.19/server-server-api/#soft-failure).
...
soft fail[soft fail](/v1.19/server-server-api/#soft-failure) or refuse to actually go forward with sending an event.
...
Because the Policy Server is not asked to sign `m.room.policy`[m.room.policy](/v1.19/client-server-api/#mroompolicy)
...
alongside the normal event signatures[event signatures](/v1.19/server-server-api/#signing-events), though uses a public
...
The Policy Server’s published signing key[published signing key](/v1.19/server-server-api/#publishing-keys) SHOULD NOT be the
...
ask for a new signature[ask for a new signature](/v1.19/server-server-api/#asking-for-a-policy-server-signature-on-an-event). If
...
event anyway, soft fail[soft fail](/v1.19/server-server-api/#soft-failure) the event.
...
Per above[above](/v1.19/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room), if a room has
...
After asking for a signature, homeservers SHOULD validate[validate](/v1.19/server-server-api/#validating-policy-server-signatures)
...
other requirements[other requirements](/v1.19/server-server-api/#policy-servers). This endpoint
is otherwise optional. See Unsupported Endpoints[Unsupported Endpoints](/v1.19/server-server-api/#unsupported-endpoints)
...
the room has enabled a Policy Server[enabled a Policy Server](/v1.19/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room).
...
but should be retained to validate the event.| Rate-limited: | Yes |
| --- | --- |
| Requires authentication: | Yes |
...
## Responses| Status | Description |
| --- | --- |
| `200` | The Policy Server has signed the event, indicating that it recommends the event for inclusion in the room. Only the Policy Server’s signature is returned. This signature is to be added to the event before sending or processing the event further. `ed25519:policy_server` is *always* used for Ed25519 signatures. |
...
| `403` | The calling server is ACL’d[ACL’d](/v1.19/server-server-api/#server-access-control-lists-acls). This endpoint is optionally affected by server ACLs. The Policy Server MAY NOT apply an ACL to the request, especially if the Policy Server is not tracking the room DAG. |
| `404` | The server is not a Policy Server, or the room is unknown/not protected by the Policy Server. Error codes include: - `M_NOT_FOUND` - The room is not known or is not protected by the Policy Server. - `M_UNRECOGNIZED` - The server is not a Policy Server. See Unsupported Endpoints[Unsupported Endpoints](/v1.19/server-server-api/#unsupported-endpoints) for more detail. |
| `429` | This request was rate-limited. |
...
Error
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `errcode` | `string` | **Required: **The `M_FORBIDDEN` error code. |
| `error` | `string` | A human-readable error message. |
...
Base64[Unpadded
Base64](/v1.19/appendices/#unpadded-base64) and stored in the event
object, in a `hashes` object, under a `sha256` key.

The event object is then *redacted*, following the redaction
algorithm[redaction
algorithm](/v1.19/client-server-api/#redactions).
Finally it is signed as described in Signing
JSON[Signing
JSON](/v1.19/appendices/#signing-json), using the server’s signing key
(see also Retrieving server keys[Retrieving server keys](/v1.19/server-server-api/#retrieving-server-keys)).
...
**[Added in `v1.18`]**
Events sent in rooms with Policy Servers[Policy Servers](/v1.19/server-server-api/#policy-servers) MUST ask[ask](/v1.19/server-server-api/#asking-for-a-policy-server-signature-on-an-event)
...
redaction algorithm[redaction algorithm](/v1.19/client-server-api/#redactions), and
...
signature[Checking for a
signature](/v1.19/appendices/#checking-for-a-signature). Note that this
...
In particular, the policy server key[policy server key](/v1.19/server-server-api/#validating-policy-server-signatures) is not
...
**[Added in `v1.18`]**
Events sent in rooms with Policy Servers[Policy Servers](/v1.19/server-server-api/#policy-servers) have additional[additional](/v1.19/server-server-api/#validating-policy-server-signatures)
...
specification[room version
specification](/v1.19/rooms/) for more information. It is
...
JSON[Canonical
JSON](/v1.19/appendices/#canonical-json).
...
JSON[Canonical
JSON](/v1.19/appendices/#canonical-json), and the JSON is hashed using
SHA-256.
...
proposals like
MSC1228[MSC1228](https://github.com/matrix-org/matrix-spec-proposals/issues/1228) will
address this issue.
