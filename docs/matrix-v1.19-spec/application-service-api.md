...
| `receive_ephemeral` | `boolean` | Whether the application service wants to receive ephemeral data[receive ephemeral data](/v1.19/application-service-api/#pushing-ephemeral-data). Defaults to `false` if not present. **Added in `v1.13`** |
...
The format of the `Authorization` header is similar to the Client-Server API[Client-Server API](/v1.19/client-server-api/#client-authentication):
...
| `type` | `string` | **Required: **The type of event, as defined by the event type specification[the event type specification](/v1.19/client-server-api/#types-of-room-events). |
...
| `event_id` | `Event ID[Event ID](/v1.19/appendices#event-ids)` | **Required: **The globally unique identifier for this event. |
...
| `room_id` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The ID of the room associated with this event. |
| `sender` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **Contains the fully-qualified ID of the user who sent this event. |
...
| `membership` | `string` | The room membership of the user making the request, at the time of the event. This property is the value of the `membership` property of the requesting user’s `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember) state at the point of the event, including any changes caused by the event. If the user had yet to join the room at the time of the event (i.e, they have no `m.room.member` state), this property is set to `leave`. Homeservers SHOULD populate this property wherever practical, but they MAY omit it if necessary (for example, if calculating the value is expensive, servers might choose to only implement it in encrypted rooms). The property is *not* normally populated in events pushed to application services via the application service transaction API (where there is no clear definition of “requesting user”). **Added in `v1.11`** |
| `prev_content` | `EventContent` | The `content` of the previous state event that was replaced by this event. This field is generated by the local homeserver, and is only returned if the event is a state event, and the client has permission to see the previous event. **Changed in `v1.2`:** Previously, this field was specified at the top level of returned events rather than in `unsigned` (with the exception of the `GET .../notifications`[GET .../notifications](/v1.19/client-server-api/#get_matrixclientv3notifications) endpoint), though in practice no known server implementations honoured this. |
| `redacted_because` | `ClientEvent` | The event that redacted this event, if any. |
| `replaces_state` | `Event ID[Event ID](/v1.19/appendices#event-ids)` | The event ID of the state event replaced by this event. This field is generated by the local homeserver, and is only returned if the event is a state event. Unlike `prev_content`, this field is included regardless of history visibility. **Added in `v1.19`** |
| `transaction_id` | `string` | The client-supplied transaction ID[transaction ID](/v1.19/client-server-api/#transaction-identifiers), for example, provided via `PUT /_matrix/client/v3/rooms/{roomId}/send/{eventType}/{txnId}`, if the client being given the event is the same one which sent it. |
...
If the `receive_ephemeral` settings is enabled in the registration[registration](/v1.19/application-service-api/#registration)
...
`presence` and `ephemeral` sections of the client-server `/sync`[/sync](/v1.19/client-server-api/#get_matrixclientv3sync)
API.
...
- **`m.presence`[m.presence](/v1.19/client-server-api/#mpresence)**: MUST be sent to the
...
- **`m.typing`[m.typing](/v1.19/client-server-api/#mtyping)**: MUST be sent to the application
...
- **`m.receipt`[m.receipt](/v1.19/client-server-api/#mreceipt)**: MUST be sent to the
...
room that they were sent in. Private read receipts[Private read receipts](/v1.19/client-server-api/#private-read-receipts)
...
`/_matrix/client/v1/appservice/{appserviceId}/ping`[/_matrix/client/v1/appservice/{appserviceId}/ping](/v1.19/application-service-api/#post_matrixclientv1appserviceappserviceidping)
...
`POST /_matrix/client/v1/appservice/{appserviceId}/ping`[POST /_matrix/client/v1/appservice/{appserviceId}/ping](/v1.19/application-service-api/#post_matrixclientv1appserviceappserviceidping).| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
- `PUT /rooms/{roomId}/send/{eventType}/{txnId}`[PUT /rooms/{roomId}/send/{eventType}/{txnId}](/v1.19/client-server-api/#put_matrixclientv3roomsroomidsendeventtypetxnid)

- `PUT /rooms/{roomId}/state/{eventType}/{stateKey}`[PUT /rooms/{roomId}/state/{eventType}/{stateKey}](/v1.19/client-server-api/#put_matrixclientv3roomsroomidstateeventtypestatekey)
...
the Legacy Authentication API[Legacy Authentication API](/v1.19/client-server-api/#legacy-api).
...
Similarly, logging in as users using the Legacy authentication API[Legacy authentication API](/v1.19/client-server-api/#legacy-api)
...
(see Using access tokens[Using access tokens](/v1.19/client-server-api/#using-access-tokens)).
...
pinging[pinging](/v1.19/application-service-api/#pinging) mechanism described above.
...
`/_matrix/app/v1/ping`[/_matrix/app/v1/ping](/v1.19/application-service-api/#post_matrixappv1ping) endpoint on the
...
| --- | --- | --- |
| `duration_ms` | `integer` | **Required: **The duration in milliseconds that the `/_matrix/app/v1/ping`[/_matrix/app/v1/ping](/v1.19/application-service-api/#post_matrixappv1ping) request took from the homeserver’s point of view. |
...
visibility API[visibility API](/v1.19/client-server-api/#put_matrixclientv3directorylistroomroomid)
...
homeservers that only support the OAuth 2.0 API[OAuth 2.0 API](/v1.19/client-server-api/#oauth-20-api).

##### Creating devices

Application services can use the `PUT /_matrix/client/v3/devices/{deviceId}`[PUT /_matrix/client/v3/devices/{deviceId}](/v1.19/client-server-api/#put_matrixclientv3devicesdeviceid)
...
Authentication](/v1.19/client-server-api/#user-interactive-authentication-api) when
used by an application service:


- `DELETE /_matrix/client/v3/devices/{deviceId}`[DELETE /_matrix/client/v3/devices/{deviceId}](/v1.19/client-server-api/#delete_matrixclientv3devicesdeviceid)

- `POST /_matrix/client/v3/delete_devices`[POST /_matrix/client/v3/delete_devices](/v1.19/client-server-api/#post_matrixclientv3delete_devices)
...
keys, so the `POST /_matrix/client/v3/keys/device_signing/upload`[POST /_matrix/client/v3/keys/device_signing/upload](/v1.19/client-server-api/#post_matrixclientv3keysdevice_signingupload)
endpoint MUST NOT require User-Interactive Authentication[User-Interactive Authentication](/v1.19/client-server-api/#user-interactive-authentication-api)
