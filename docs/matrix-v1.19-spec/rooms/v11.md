...
This room version builds on version 10[version 10](/v1.19/rooms/v10/) while clarifying redaction
rules.
...
**[New in this version]** The top-level `origin`, `membership`, and `prev_state` properties
are no longer protected from redaction. The `m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate)
event now keeps the entire `content` property. The `m.room.redaction`[m.room.redaction](/v1.19/client-server-api/#mroomredaction)
event keeps the `redacts` property under `content`. The
`m.room.power_levels`[m.room.power_levels](/v1.19/client-server-api/#mroompower_levels) event keeps the
...
`m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate) events. In all room versions,
clients can rely on `sender` instead to determine a room creator.

The format of `m.room.redaction`[m.room.redaction](/v1.19/client-server-api/#mroomredaction)
events has been modified. Client should look for the `redacts` key under `content`
instead of a top-level event property.

The `third_party_invite` key of `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember)
...
See above[See above](/v1.19/rooms/v11/#redactions).

### Event format

The core event format is the same as room version 10[room version 10](/v1.19/rooms/v10/#event-format).
...
A persistent data unit (event) for room version 11.
...
the Handling redactions[Handling redactions](/v1.19/rooms/v11/#handling-redactions) section.

The rules are as follows:


1. **[Changed in this version]**
...
selection](/v1.19/server-server-api/#auth-events-selection)
