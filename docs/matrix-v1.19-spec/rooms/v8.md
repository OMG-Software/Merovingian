...
This room version builds on version 7[version 7](/v1.19/rooms/v7/) to introduce a new
...
join events. Room version 9[Room version 9](/v1.19/rooms/v9/) should be preferred over v8 when
creating rooms.
...
Client-Server API[Client-Server API](/v1.19/client-server-api/#restricted-rooms).

Clients which implement the redaction algorithm locally should refer to the
redactions[redactions](/v1.19/rooms/v8/#redactions) section below for a full overview.

### Redactions

**[New in this version]** `m.room.join_rules` events now keep `allow` in addition to other
keys in `content` when being redacted.

Room version 9[Room version 9](/v1.19/rooms/v9/) adds additional cases of protected properties for behaviour
...
Room version 7[Room version 7](/v1.19/rooms/v7/).

### Authorisation rules

**[New in this version]** For checks performed upon `m.room.member` events, new
...
See above[See above](/v1.19/rooms/v8/#redactions).
