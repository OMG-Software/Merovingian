...
This room version builds on version 2[version 2](/v1.19/rooms/v2/) with an improved event
format.
...
redaction algorithm locally should refer to the redactions[redactions](/v1.19/rooms/v3/#redactions)
...
all the remaining behaviour described by room version 2[room version 2](/v1.19/rooms/v2/).

### Handling redactions

**[New in this version]** In room versions 1 and 2, redactions were
...
Base64[Unpadded
Base64](/v1.19/appendices/#unpadded-base64), prefixed with `$`. A
...
`GET /_matrix/federation/v1/make_join/{roomId}/{userId}`[GET /_matrix/federation/v1/make_join/{roomId}/{userId}](/v1.19/server-server-api/#get_matrixfederationv1make_joinroomiduserid).)
...
A persistent data unit (event) for room version 3.
...
of Canonical JSON[Canonical JSON](/v1.19/appendices/#canonical-json), power levels can be formatted as floats.
...
the Handling Redactions[Handling Redactions](/v1.19/rooms/v3/#handling-redactions) section.
...
- `m.room.aliases`[m.room.aliases](/v1.19/client-server-api/#historical-events) allows key `aliases`.
...
appendices[appendices](/v1.19/appendices/#canonical-json) for the reasons
described there.
