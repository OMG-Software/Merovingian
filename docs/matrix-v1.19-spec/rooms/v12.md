...
This room version builds on version 11[version 11](/v1.19/rooms/v11/), iterating on the state resolution
...
`m.room.create` event, per below. See the room ID grammar[room ID grammar](/v1.19/appendices/#room-ids)
...
A persistent data unit (event) for room version 12 and beyond.
...
| --- | --- | --- |
| `auth_events` | `[string]` | **Required: **Event IDs for the authorisation events that would allow this event to be in the room. Must contain less than or equal to 10 events. Note that if the relevant auth event selection rules are used, this restriction should never be encountered. |
| `content` | `object` | **Required: **The content of the event. |
| `depth` | `integer` | **Required: **The maximum depth of the `prev_events`, plus one. Must be less than the maximum value for an integer (2^53 - 1). If the room’s depth is already at the limit, the depth must be set to the limit. |
| `hashes` | `Event HashEvent Hash` | **Required: **Content hashes of the PDU, following the algorithm specified in Signing Events[Signing Events](/v1.19/server-server-api/#signing-events). |
...
| `room_id` | `string` | **Required: **Room identifier. Omitted from `m.room.create` events. |
| `sender` | `string` | **Required: **The ID of the user sending the event. |
| `signatures` | `{string: {string: string}}` | **Required: **Signatures for the PDU, following the algorithm specified in Signing Events[Signing Events](/v1.19/server-server-api/#signing-events). |
...
- `m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate)

- `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember)

- `m.room.join_rules`[m.room.join_rules](/v1.19/client-server-api/#mroomjoin_rules)

- `m.room.power_levels`[m.room.power_levels](/v1.19/client-server-api/#mroompower_levels)

- `m.room.third_party_invite`[m.room.third_party_invite](/v1.19/client-server-api/#mroomthird_party_invite)
...
**[New in this version]** The power level of “room creators” is infinitely high.
...
the Handling redactions[Handling redactions](/v1.19/rooms/v12/#handling-redactions) section.
...
**[Changed in this version]** If the event has a `room_id`, reject.
...
If `content.room_version` is present and is not a recognised
version, reject.
...
**[New in this version]** If `additional_creators` is present in `content` and
is not an array of strings where each string passes the same user ID[user ID](/v1.19/appendices/#user-identifiers)
...
2. **[New in this version]** If the event’s `room_id` is not an event ID for an accepted
...
**[Changed in this version]** If there are entries whose `type` and `state_key`
don’t match those specified by the auth events selection[auth events selection](/v1.19/server-server-api/#auth-events-selection)
...
PDU[checks
performed on receipt of a
PDU](/v1.19/server-server-api/#checks-performed-on-receipt-of-a-pdu), reject.
...
are valid user IDs with values that are integers, reject.
  ...
  4. **[New in this version]** If the `users` property in `content` contains the
...
**[New in this version]** This state resolution algorithm is largely the same as the
algorithm found in room version 2[room version 2](/v1.19/rooms/v2/) with the following modifications:


1. 


The *iterative auth checks algorithm* in the Algorithm[Algorithm](/v1.19/rooms/v12/#algorithm) subsection
...
A new definition[definition](/v1.19/rooms/v12/#definitions) for *conflicted state subgraph* has been added
...
To ensure the new conflicted state subgraph is actually referenced, the definition
...
**[New in this version]** **Conflicted state subgraph.**
...
**[Changed in this version]** **Full conflicted set.**
...
rules[authorisation
rules](/v1.19/server-server-api/#authorisation-rules).
...
2. **[Changed in this version]** Apply the *iterative auth checks algorithm*,
...
- `m.room.member`[m.room.member](/v1.19/client-server-api/#mroommember) allows keys `membership`, `join_authorised_via_users_server`.
Additionally, it allows the `signed` key of the `third_party_invite` key.

- `m.room.create`[m.room.create](/v1.19/client-server-api/#mroomcreate) allows all keys.

- `m.room.join_rules`[m.room.join_rules](/v1.19/client-server-api/#mroomjoin_rules) allows keys `join_rule`, `allow`.

- `m.room.power_levels`[m.room.power_levels](/v1.19/client-server-api/#mroompower_levels) allows keys
`ban`, `events`, `events_default`, `invite`, `kick`, `redact`, `state_default`,
`users`, `users_default`.

- `m.room.history_visibility`[m.room.history_visibility](/v1.19/client-server-api/#mroomhistory_visibility)
allows key `history_visibility`.

- `m.room.redaction`[m.room.redaction](/v1.19/client-server-api/#mroomredaction) allows key `redacts`.


### Handling redactions

In room versions 1 and 2, redactions were
explicitly part of the authorisation rules[authorisation rules](/v1.19/rooms/v1/#authorisation-rules)
under Rule 11. As of room version 3, these conditions no longer exist as
represented by this version’s authorisation rulesthis version’s authorisation rules.
...
hash[reference
hash](/v1.19/server-server-api/#calculating-the-reference-hash-for-an-event) of
...
appendices[appendices](/v1.19/appendices/#canonical-json). This translates to a
...
/_matrix/key/v2/server[GET
/_matrix/key/v2/server](/v1.19/server-server-api/#get_matrixkeyv2server)
or POST
/_matrix/key/v2/query[POST
/_matrix/key/v2/query](/v1.19/server-server-api/#post_matrixkeyv2query)
