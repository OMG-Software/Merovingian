...
RFC 4648[RFC 4648](https://tools.ietf.org/html/rfc4648), without “=” padding.
...
as the 63rd. This matches RFC4648’s definition of URL-safe base64[RFC4648’s definition of URL-safe base64](https://tools.ietf.org/html/rfc4648#section-5).
...
bytes using Canonical JSON[Canonical JSON](/v1.19/appendices/#canonical-json), computing the signature
...
Adapted grammar from http://tools.ietf.org/html/rfc7159[http://tools.ietf.org/html/rfc7159](http://tools.ietf.org/html/rfc7159) removing
insignificant whitespace, fractions, exponents and redundant character escapes
written in ABNF[ABNF](https://datatracker.ietf.org/doc/html/rfc5234) with
case sensitive strings[case sensitive strings](https://datatracker.ietf.org/doc/html/rfc7405).
...
signature is encoded using unpadded Base64[unpadded Base64](/v1.19/appendices/#unpadded-base64). The resulting base64
...
(http://nacl.cr.yp.to/[http://nacl.cr.yp.to/](http://nacl.cr.yp.to/)). The *key identifier* is used to distinguish
...
JSON[Canonical
JSON](/v1.19/appendices/#canonical-json) encoding.
...
the room versions specification[room versions specification](/v1.19/rooms/) for more
information.
...
RFC3513, section 2.2[RFC3513, section 2.2](https://tools.ietf.org/html/rfc3513#section-2.2).
...
where `domain` is the server name[server name](/v1.19/appendices/#server-name) of the homeserver
...
Event IDs[Event IDs](/v1.19/appendices/#event-ids) section below for more information.
...
The `domain` of a user ID is the server name[server name](/v1.19/appendices/#server-name) of the
...
**[Changed in `v1.16`]** Room IDs can now appear without a domain depending on
the room version.
...
However, the precise format depends upon the room version specification[room version specification](/v1.19/rooms/):
...
(`A-Z`, `a-z`, `0-9`) when generating them. The `domain` is the server name[server name](/v1.19/appendices/#server-name)
...
event ID[event ID](/v1.19/appendices/#event-ids) of the `m.room.create` event using `!` as the sigil instead
...
parse the user IDs[user IDs](/v1.19/appendices/#user-identifiers) found in the `m.room.create` event’s `sender`.
...
The `domain` of a room alias is the server name[server name](/v1.19/appendices/#server-name) of the
...
specification](/v1.19/rooms/): early room versions included a `domain` component,
...
Note that this format is deliberately following RFC 3986[RFC 3986](https://tools.ietf.org/html/rfc3986)
to ensure maximum compatibility with existing tooling. The scheme name (`matrix`) is
registered alongside other schemes by the IANA here[here](https://www.iana.org/assignments/uri-schemes/uri-schemes.xhtml).
...
Common Identifier Format[Common Identifier Format](/v1.19/appendices/#common-identifier-format).
...
routing Room IDs is described below[below](/v1.19/appendices/#routing), and is encouraged for use in
...
way is being worked out, such as one proposed by MSC3020[MSC3020](https://github.com/matrix-org/matrix-spec-proposals/pull/3020).


Custom query parameters can be specified using the
Common Namespaced Identifier format[Common Namespaced Identifier format](/v1.19/appendices/#common-namespaced-identifier-grammar) and
...
original MSC[original MSC](https://github.com/matrix-org/matrix-spec-proposals/blob/main/proposals/2312-matrix-uri.md#recommended-implementation).
...
defined in RFC 3986[RFC 3986](https://tools.ietf.org/html/rfc3986):
...
versions[room
versions](/v1.19/rooms/). These slashes should normally be
...
the specification and is subsequently replaced with Spaces[Spaces](/v1.19/client-server-api/#spaces). Historical
...
Client Server `/join/{roomIdOrAlias}` API[Client Server /join/{roomIdOrAlias} API](/v1.19/client-server-api/#post_matrixclientv3joinroomidoralias).
...
be the email address, *e.g.* the string `[email protected][[email protected]](/cdn-cgi/l/email-protection)`. Since domain
resolution is case-insensitive, the email address `[email protected][[email protected]](/cdn-cgi/l/email-protection)` is
also has the 3PID address of `[email protected][[email protected]](/cdn-cgi/l/email-protection)` (without the capital ’e’)
rather than `[email protected][[email protected]](/cdn-cgi/l/email-protection)`.
...
chapter 5 of the unicode standard[chapter 5 of the unicode standard](https://www.unicode.org/versions/Unicode13.0.0/ch05.pdf#G21790).
For example, `Strauß@Example.com` must be considered to be `[email protected][[email protected]](/cdn-cgi/l/email-protection)`
