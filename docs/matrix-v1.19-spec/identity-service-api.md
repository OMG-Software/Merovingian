...
3PID types are described in 3PID Types[3PID Types](/v1.19/appendices/#3pid-types)
Appendix.
...
All `POST` and `PUT` endpoints, with the exception (for historical reasons) of `POST /_matrix/identity/v2/account/logout`[POST /_matrix/identity/v2/account/logout](/v1.19/identity-service-api/#post_matrixidentityv2accountlogout),
...
the Specification Versioning[the Specification Versioning](/v1.19/#specification-versions) for more
information.

All supported versions, including patch versions, are reported by the server.| Rate-limited: | No |
| --- | --- |
...
Gets information about what user owns the access token used in the request.| Rate-limited: | No |
| --- | --- |
...
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service). |
...
future requests to the server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
returned by `/openid/request_token` in the Client-Server API.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
to avoid presenting the user with terms they have already agreed to.| Event type: | Message event |
| --- | --- |
...
user. Note that this endpoint does not require authentication.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `policies` | `{string: Policy ObjectPolicy Object}` | **Required: **The policies the server offers. Mapped from arbitrary ID (unused in this version of the specification) to a Policy Object. |
...
| <Other properties> | `Internationalised PolicyInternationalised Policy` | The policy information for the specified language. |
...
may not be accepting all terms at once.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
by entities acting as a client for the identity server.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
JSON[Signing
JSON](/v1.19/appendices/#signing-json) algorithm applies.
...
Check whether a short-term public key is valid.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **The unpadded Base64[unpadded Base64](/v1.19/appendices/#unpadded-base64)-encoded public key to check. This MUST be the exact same encoded string returned in the response of the `/store-invite`[/store-invite](/v1.19/identity-service-api/#post_matrixidentityv2store-invite) endpoint, or found in the corresponding `m.room.third_party_invite`[m.room.third_party_invite](/v1.19/client-server-api/#mroomthird_party_invite) event, so it may use the standard or URL-safe alphabets. |
...
be the same, provided the key exists.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **The unpadded Base64[unpadded Base64](/v1.19/appendices/#unpadded-base64)-encoded public key to check. |
...
Get the public key for the passed key ID.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | No |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **Unpadded Base64[Unpadded Base64](/v1.19/appendices/#unpadded-base64)-encoded public key. |
...
any of the algorithms defined in this specification.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `lookup_pepper` | `string` | **Required: **The pepper the client MUST use in hashing identifiers, and MUST supply to the `/lookup` endpoint when performing lookups. Servers SHOULD rotate this string often. |
...
later in this specification.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `addresses` | `[string]` | **Required: **The addresses to look up. The format of the entries here depend on the `algorithm` used. Note that queries which have been incorrectly hashed or formatted will lead to no matches. Note that addresses are case sensitive: review the 3PID Types[3PID Types](/v1.19/appendices/#3pid-types) to verify the intended case an identifier should be prior to submission/hashing. |
...
`[email protected][[email protected]](/cdn-cgi/l/email-protection)`’s bindings, it would first format the query as
`[email protected][[email protected]](/cdn-cgi/l/email-protection) email ThePepperGoesHere`.
...
defined by RFC 4634[RFC 4634](https://tools.ietf.org/html/rfc4634). The
resulting bytes are then encoded using URL-Safe unpadded
Base64[URL-Safe unpadded
Base64](/v1.19/appendices/#url-safe-unpadded-base64).
...
`[email protected][[email protected]](/cdn-cgi/l/email-protection)`’s bindings, it would format the query as
`[email protected][[email protected]](/cdn-cgi/l/email-protection) email`.
...
MSC2134[MSC2134](https://github.com/matrix-org/matrix-spec-proposals/blob/main/proposals/2134-identity-hash-lookup.md) has much
...
| `email` | `Email Address[Email Address](https://datatracker.ietf.org/doc/html/rfc5321#section-4.1.2)` | **Required: **The email address to validate. |
| `next_link` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | Optional. When the validation is completed, the identity server will redirect the user to this URL. This option is ignored when submitting 3PID validation information through a POST request. |
...
| --- | --- |
| `200` | Session created. |
| `400` | An error occurred. Some possible errors are: - `M_INVALID_EMAIL`: The email address provided was invalid. - `M_EMAIL_SEND_ERROR`: The validation email could not be sent. |
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service). |
...
used by end-users, and so the response should be human-readable.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| --- | --- |
| `200` | Session created. |
| `400` | An error occurred. Some possible errors are: - `M_INVALID_ADDRESS`: The phone number provided was invalid. - `M_SEND_ERROR`: The validation SMS could not be sent. - `M_DESTINATION_REJECTED`: The identity server cannot deliver an SMS to the provided country or region. |
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service). |
...
Determines if a given 3pid has been validated by a user.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `400` | The session has not been validated. If the session has not been validated, then `errcode` will be `M_SESSION_NOT_VALIDATED`. If the session has timed out, then `errcode` will be `M_SESSION_EXPIRED`. |
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service). |
| `404` | The Session ID or client secret were not found. |
...
homeserver is acting on behalf of a client.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `threepid` | `3PID3PID` | **Required: **The 3PID to remove. Must match the 3PID used to generate the session if using `sid` and `client_secret` to authenticate this request. |
...
| `medium` | `string` | **Required: **A medium from the 3PID Types[3PID Types](/v1.19/appendices/#3pid-types) Appendix, matching the medium of the identifier to unbind. |
...
| `403` | The credentials supplied to authenticate the request were invalid. This may also be returned if the identity server does not support the chosen authentication method (such as blocking homeservers from unbinding identifiers). Another common error code is `M_TERMS_NOT_SIGNED` where the user needs to agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service) in order to continue. |
...
/3pid/onbind[/3pid/onbind](/v1.19/server-server-api/#put_matrixfederationv13pidonbind)
...
requests to `/_matrix/identity/v2/pubkey/ephemeral/isvalid`[/_matrix/identity/v2/pubkey/ephemeral/isvalid](/v1.19/identity-service-api/#get_matrixidentityv2pubkeyephemeralisvalid).
...
the `address` of the pending invite for display purposes.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `room_alias` | `Room Alias[Room Alias](/v1.19/appendices#room-aliases)` | The Matrix room alias for the room to which the user is invited. This should be retrieved from the `m.room.canonical_alias` state event. |
| `room_avatar_url` | `Matrix Content URI[Matrix Content URI](/v1.19/client-server-api#matrix-content-mxc-uris)` | The Content URI for the room to which the user is invited. This should be retrieved from the `m.room.avatar` state event. |
| `room_id` | `Room ID[Room ID](/v1.19/appendices#room-ids)` | **Required: **The Matrix room ID to which the user is invited |
...
| `sender` | `User ID[User ID](/v1.19/appendices#user-identifiers)` | **Required: **The Matrix user ID of the inviting user |
| `sender_avatar_url` | `Matrix Content URI[Matrix Content URI](/v1.19/client-server-api#matrix-content-mxc-uris)` | The Content URI for the avatar of the user ID initiating the invite. |
...
| `400` | An error has occurred. If the 3pid is already bound to a Matrix user ID, the error code will be `M_THREEPID_IN_USE`. If the medium is unsupported, the error code will be `M_UNRECOGNIZED`. |
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.19/identity-service-api/#terms-of-service). |
...
| `public_keys` | `[PublicKeyPublicKey]` | **Required: **A list of [server’s long-term public key, generated ephemeral public key]. |
...
PublicKey
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `key_validity_url` | `string` | **Required: **The URI of an endpoint where the validity of this key can be checked by passing it as a `public_key` query parameter. See key management[key management](/v1.19/identity-service-api/#key-management). |
| `public_key` | `string` | **Required: **The public key, encoded using standard unpadded Base64[standard unpadded Base64](/v1.19/appendices/#unpadded-base64) or URL-safe unpadded Base64[URL-safe unpadded Base64](/v1.19/appendices/#url-safe-unpadded-base64). |
...
to `store-invite`, and fetch the sender of the invite.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `private_key` | `string` | **Required: **The private key, encoded as Unpadded base64[Unpadded base64](/v1.19/appendices/#unpadded-base64). |
