...
| Federation | `runtime-wired` | Room-version-specific PDU verification is now conformance-covered for every stable version v1-v12 (reference-hash event IDs, v12 implicit create/auth_events rules). Remaining: simultaneously-active multiple signing keys and broader Matrix federation conformance coverage. Key-rotation publication with `old_verify_keys` landed in 0.8.6. |
