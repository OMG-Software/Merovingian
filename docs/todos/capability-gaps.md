...
| Client-server API | `runtime-wired` | Sync surfaces (long polling, filters, presence, to-device, account-data, sliding sync) now wired. Remaining: OIDC support (`auth_metadata`), TURN credential issuance, remote-thumbnail conformance fixtures, and third-party invite auth. |
...
| `GET /_matrix/federation/v1/media/download/{mediaId}` inbound (serving our media to remote servers) | `missing` | Not implemented — this server has no route for the endpoint, on either the authenticated (v1.11) or deprecated `/_matrix/media/v3/download/{serverName}/{mediaId}` path, for *inbound* federation requests. The consuming side (fetching remote media as a client of another server) was fixed in 0.10.34; serving our own locally-uploaded media to other homeservers over federation still needs its own route, X-Matrix authentication, and multipart/mixed response encoding. |
