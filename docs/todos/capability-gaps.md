...
| Client-server API | `runtime-wired` | Sync surfaces (long polling, filters, presence, to-device, account-data, sliding sync) now wired. Static TURN credentials and OIDC discovery metadata (`auth_metadata`) are implemented. Remaining: third-party invite auth. |
