...
| Application Service API | **Closed in 0.12.1** — registration files, `as_token`/`hs_token`, `m.login.application_service`, namespace exclusivity, outbound transaction delivery, all six `GET /_matrix/client/v3/thirdparty/*` third-party lookup routes (backed by outbound `GET /_matrix/app/v1/thirdparty/*` calls, with multi-appservice aggregation and unreachable-appservice degradation), and the outbound `GET /_matrix/app/v1/users/{userId}` / `/rooms/{roomAlias}` query hooks are all implemented — see CHANGELOG 0.12.1 and `docs/todos/capability-gaps.md`, "Application service API". The query hooks are invoked on a local miss: `GET /directory/room/{roomAlias}` and `GET /profile/{userId}` consult the appservice whose namespace covers the identifier and re-check locally afterwards (wired in 0.12.1, covered by `test_appservice_query_hooks_flow.cpp`) | Closed — see CHANGELOG 0.12.1 |
