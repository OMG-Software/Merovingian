...
Complement/property/load/chaos conformance — plus the still-open rows in the
functional-holes table above: push production-readiness, `/messages` state
divergence, and `/search` at scale. HTTP keep-alive, tiered rate limiting, the
Application Service API and SSO login closed in 0.12.1; the global runtime lock
is partially closed (see that row for what remains).
