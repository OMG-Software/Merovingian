...
3PID types are described in 3PID Types[3PID Types](/v1.18/appendices/#3pid-types)
Appendix.
...
All `POST` and `PUT` endpoints, with the exception (for historical reasons) of `POST /_matrix/identity/v2/account/logout`[POST /_matrix/identity/v2/account/logout](/v1.18/identity-service-api/#post_matrixidentityv2accountlogout),
...
the Specification Versioning[the Specification Versioning](/v1.18/#specification-versions) for more
information.
...
| `403` | The user must do something in order to use this endpoint. One example is an `M_TERMS_NOT_SIGNED` error where the user must agree to more terms[agree to more terms](/v1.18/identity-service-api/#terms-of-service). |
...
JSON[Signing
JSON](/v1.18/appendices/#signing-json) algorithm applies.
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **The unpadded Base64[unpadded Base64](/v1.18/appendices/#unpadded-base64)-encoded public key to check. This MUST be the exact same encoded string returned in the response of the `/store-invite`[/store-invite](/v1.18/identity-service-api/#post_matrixidentityv2store-invite) endpoint, or found in the corresponding `m.room.third_party_invite`[m.room.third_party_invite](/v1.18/client-server-api/#mroomthird_party_invite) event, so it may use the standard or URL-safe alphabets. |
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **The unpadded Base64[unpadded Base64](/v1.18/appendices/#unpadded-base64)-encoded public key to check. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `public_key` | `string` | **Required: **Unpadded Base64[Unpadded Base64](/v1.18/appendices/#unpadded-base64)-encoded public key. |
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `addresses` | `[string]` | **Required: **The addresses to look up. The format of the entries here depend on the `algorithm` used. Note that queries which have been incorrectly hashed or formatted will lead to no matches. Note that addresses are case sensitive: review the 3PID Types[3PID Types](/v1.18/appendices/#3pid-types) to verify the intended case an identifier should be prior to submission/hashing. |
...
Base64[Unpadded
Base64](/v1.18/appendices/#unpadded-base64) (similar to room version
4’s event ID format[room version
4’s event ID format](/v1.18/rooms/v4/#event-ids)).
...
| `medium` | `string` | **Required: **A medium from the 3PID Types[3PID Types](/v1.18/appendices/#3pid-types) Appendix, matching the medium of the identifier to unbind. |
...
| `403` | The credentials supplied to authenticate the request were invalid. This may also be returned if the identity server does not support the chosen authentication method (such as blocking homeservers from unbinding identifiers). Another common error code is `M_TERMS_NOT_SIGNED` where the user needs to agree to more terms[agree to more terms](/v1.18/identity-service-api/#terms-of-service) in order to continue. |
...
/3pid/onbind[/3pid/onbind](/v1.18/server-server-api/#put_matrixfederationv13pidonbind)
...
requests to `/_matrix/identity/v2/pubkey/ephemeral/isvalid`[/_matrix/identity/v2/pubkey/ephemeral/isvalid](/v1.18/identity-service-api/#get_matrixidentityv2pubkeyephemeralisvalid).
...
| `room_alias` | `Room Alias[Room Alias](/v1.18/appendices#room-aliases)` | The Matrix room alias for the room to which the user is invited. This should be retrieved from the `m.room.canonical_alias` state event. |
| `room_avatar_url` | `Matrix Content URI[Matrix Content URI](/v1.18/client-server-api#matrix-content-mxc-uris)` | The Content URI for the room to which the user is invited. This should be retrieved from the `m.room.avatar` state event. |
| `room_id` | `Room ID[Room ID](/v1.18/appendices#room-ids)` | **Required: **The Matrix room ID to which the user is invited |
...
| `sender` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The Matrix user ID of the inviting user |
| `sender_avatar_url` | `Matrix Content URI[Matrix Content URI](/v1.18/client-server-api#matrix-content-mxc-uris)` | The Content URI for the avatar of the user ID initiating the invite. |
...
PublicKey
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `key_validity_url` | `string` | **Required: **The URI of an endpoint where the validity of this key can be checked by passing it as a `public_key` query parameter. See key management[key management](/v1.18/identity-service-api/#key-management). |
| `public_key` | `string` | **Required: **The public key, encoded using standard or URL-safe unpadded Base64[unpadded Base64](/v1.18/appendices/#unpadded-base64). |
...
| `private_key` | `string` | **Required: **The private key, encoded as Unpadded base64[Unpadded base64](/v1.18/appendices/#unpadded-base64). |
