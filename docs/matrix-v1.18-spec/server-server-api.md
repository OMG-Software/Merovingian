...
grammar[grammar](/v1.18/appendices/#server-name). Where applicable, a delegated
...
server name[server name](/v1.18/appendices/#server-name) by making a
...
| --- | --- | --- |
| `m.server` | `string` | The server name to delegate server-server communications to, with optional port. The delegated server name uses the same grammar as server names in the appendices[server names in the appendices](/v1.18/appendices/#server-name). |
...
| `server_name` | `string` | **Required: **The homeserver’s server name[server name](/v1.18/appendices/#server-name). |
| `signatures` | `{string: {string: string}}` | **Required: **Digital signatures for this object signed using the `verify_keys`. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.18/appendices/#signing-json). |
| `valid_until_ts` | `integer` | **Required: **POSIX timestamp in milliseconds when the list of valid keys should be refreshed. This field MUST be ignored in room versions 1, 2, 3, and 4. Keys used beyond this timestamp MUST be considered invalid, depending on the room version specification[room version specification](/v1.18/rooms/). Servers MUST use the lesser of this field and 7 days into the future when determining if a key is valid. This is to avoid a situation where an attacker publishes a key which is valid for a significant amount of time without a way for the homeserver owner to revoke it. |
...
| `key` | `string` | **Required: **The Unpadded base64[Unpadded base64](/v1.18/appendices/#unpadded-base64) encoded key. |
...
path parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `serverName` | `string` | **Required: **The server name[server name](/v1.18/appendices/#server-name) to query |
...
algorithm](/v1.18/appendices/#signing-json). The resulting signatures are added
...
section](/v1.18/server-server-api/#resolving-server-names) above do not affect these - the server
...
the room version specification[room version specification](/v1.18/rooms/) for precise event formats.| Rate-limited: | No |
| --- | --- |
...
| `pdus` | `[PDU]` | **Required: **List of persistent updates to rooms. Must not include more than 50 PDUs. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
PDU Processing Results
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `pdus` | `{Event ID[Event ID](/v1.18/appendices#event-ids): PDU Processing ResultPDU Processing Result}` | **Required: **The PDUs from the original transaction. The string key represents the ID of the PDU (event) that was processed. |
...
an event to be valid, it must comply with the event format of that room version[room version](/v1.18/rooms/).
...
section of the room version specifications[room version specifications](/v1.18/rooms/) for details on when it
is required.
...
7. **[Added in `v1.18`]** Is validated[validated](/v1.18/server-server-api/#validating-policy-server-signatures)
by the Policy Server, if the room is using a Policy Server[using a Policy Server](/v1.18/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room),
otherwise it is “soft failed”.
...
The Signing Events[Signing Events](/v1.18/server-server-api/#signing-events) section has more information on
...
the current `m.room.power_levels`[m.room.power_levels](/v1.18/client-server-api/#mroompower_levels)
...
For an `m.room.member`[m.room.member](/v1.18/client-server-api/#mroommember) state event, the
user given by the `state_key` of the event.

Some room versions[room versions](/v1.18/rooms/) accept power level values to be represented as
...
See the room version specification[room version specification](/v1.18/rooms/) for more information.
...
**[Changed in `v1.16`]** Depending on the room version[room version](/v1.18/rooms/), the
`m.room.create` event.
...
is present, and the room version supports restricted rooms[room version supports restricted rooms](/v1.18/rooms/#feature-matrix),
...
**[Added in `v1.18`]**
Events can also be soft failed if they fail Policy Server checks[Policy Server checks](/v1.18/server-server-api/#validating-policy-server-signatures).
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `auth_chain` | `[PDU]` | **Required: **The PDUs[PDUs](/v1.18/server-server-api/#pdus) forming the auth chain of the given event. The event format varies depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
the room version specification[room version specification](/v1.18/rooms/).
...
| `origin` | `string` | **Required: **The `server_name` of the homeserver sending this transaction. |
| `origin_server_ts` | `integer` | **Required: **POSIX timestamp in milliseconds on originating homeserver when this transaction started. |
| `pdus` | `[PDU]` | **Required: **List of persistent updates to rooms. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `events` | `[PDU]` | **Required: **The missing events. The event format varies depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
| `origin` | `string` | **Required: **The `server_name` of the homeserver sending this transaction. |
| `origin_server_ts` | `integer` | **Required: **POSIX timestamp in milliseconds on originating homeserver when this transaction started. |
| `pdus` | `[PDU]` | **Required: **A single PDU. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `auth_chain` | `[PDU]` | **Required: **The full set of authorization events that make up the state of the room, and their authorization events, recursively. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
| `pdus` | `[PDU]` | **Required: **The fully resolved state of the room at the given event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
client-server endpoint[client-server endpoint](/v1.18/client-server-api/#get_matrixclientv1roomsroomidtimestamp_to_event)
...
`/event/{event_id}`[/event/{event_id}](/v1.18/server-server-api/#get_matrixfederationv1eventeventid)
...
`/query/directory`[/query/directory](/v1.18/server-server-api/#get_matrixfederationv1querydirectory) API endpoint. In the case of a new user joining a
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used for the rest of the Joining Rooms[Joining Rooms](/v1.18/server-server-api/#joining-rooms) handshake. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
| `400` | The request is invalid, the room the server is attempting to join has a version that is not listed in the `ver` parameters, or the server was unable to validate restricted room conditions[restricted room conditions](/v1.18/server-server-api/#restricted-rooms). The error should be passed through to clients so that they may give better feedback to users. New in `v1.2`, the following error conditions might happen: If the room is restricted[restricted](/v1.18/client-server-api/#restricted-rooms) and none of the conditions can be validated by the server then the `errcode` `M_UNABLE_TO_AUTHORISE_JOIN` must be used. This can happen if the server does not know about any of the rooms listed as conditions, for example. `M_UNABLE_TO_GRANT_JOIN` is returned to denote that a different server should be attempted for the join. This is typically because the resident server can see that the joining user satisfies one or more conditions, such as in the case of restricted rooms[restricted rooms](/v1.18/client-server-api/#restricted-rooms), but the resident server would be unable to meet the auth rules governing `join_authorised_via_users_server` on the resulting `m.room.member` event. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `event` | `Event TemplateEvent Template` | An unsigned template event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
the room version specification[room version specification](/v1.18/rooms/) for precise event formats.
...
- The join event fails a signature check[signature check](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
client-server API[client-server API](/v1.18/client-server-api/#restricted-rooms) and are available
in room versions which support restricted join rules[which support restricted join rules](/v1.18/rooms/#feature-matrix).
...
much like joining rooms[joining rooms](/v1.18/server-server-api/#joining-rooms) the server
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used for the rest of the Knocking Rooms[Knocking Rooms](/v1.18/server-server-api/#knocking-rooms) handshake. Note that events have a different format depending on room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `event` | `Event TemplateEvent Template` | **Required: **An unsigned template event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
- The knock event fails a signature check[signature check](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `knock_room_state` | `[PDU]` | **Required: **A list of state events to help the initiator of the knock identify the room. Translated as stripped state events[stripped state events](/v1.18/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Entries which are improperly signed[improperly signed](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events) or formatted SHOULD be removed by the server prior to supplying them over the Client-Server API. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
according to the room’s version (see room version specification[room version specification](/v1.18/rooms/)). However,
...
room version specification[room version specification](/v1.18/rooms/) for precise event formats. **The request and response
...
transaction](/v1.18/server-server-api/#transactions).
...
- The invite event fails a signature check[signature check](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
- One or more events fails a signature check[signature check](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
InviteEvent
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `Membership Event ContentMembership Event Content` | **Required: **The content of the event, matching what is available in the Client-Server API[Client-Server API](/v1.18/client-server-api/). Must include a `membership` of `invite`. |
...
UnsignedData
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `invite_room_state` | `[PDU]` | A list of state events to help the receiver of the invite identify the room. Translated as stripped state events[stripped state events](/v1.18/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Servers might need to apply validation to the `invite_room_state` depending on room version. See endpoint description for more information. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
## Responses| Status | Description |
| --- | --- |
| `200` | The event with the invited server’s signature added. All other fields of the events should remain untouched. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
| `400` | The request is invalid in some way. |
| `403` | The invite is not allowed. The `M_FORBIDDEN` error code is used to indicate one of the following: - The sender is not allowed to send invites to the target user/homeserver. - The homeserver does not permit anyone to invite its users. - The homeserver refuses to participate in the room. **[Added in `v1.18`]** The `M_INVITE_BLOCKED` error code is used to indicate that the homeserver rejected the invite due to invite blocking[invite blocking](/v1.18/client-server-api/#invite-permission). |
...
Event Container
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `event` | `InviteEventInviteEvent` | **Required: **An invite event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
...
| --- | --- | --- |
| `event` | `InviteEventInviteEvent` | **Required: **An invite event. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. |
| `invite_room_state` | `[PDU]` | A list of state events to help the receiver of the invite identify the room. Translated as stripped state events[stripped state events](/v1.18/client-server-api/#stripped-state) over the Client-Server API. MUST contain the `m.room.create` event for the room. All events listed MUST additionally be formatted according to the room version specification. Servers might need to apply validation to the `invite_room_state` depending on room version. See the endpoint description for more information. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **Changed in `v1.16`:** `m.room.create` and format requirements were added. |
...
Similar to the Joining Rooms[Joining Rooms](/v1.18/server-server-api/#joining-rooms) handshake, the server
...
## Responses| Status | Description |
| --- | --- |
| `200` | A template to be used to call `/send_leave`. Note that events have a different format depending on the room version - check the room version specification[room version specification](/v1.18/rooms/) for precise event formats. **The response body here describes the common event fields in more detail and may be missing other required fields for a PDU.** |
...
- The leave event fails a signature check[signature check](/v1.18/server-server-api/#validating-hashes-and-signatures-on-received-events).
...
under the Third-party invites[Third-party invites](/v1.18/client-server-api/#third-party-invites) module.
...
API](/v1.18/identity-service-api/).
...
request[lookup
request](/v1.18/identity-service-api/#post_matrixidentityv2lookup) on the identity
server will return it. The invite is then processed by the inviting homeserver
as a standard `m.room.member` invite event[standard m.room.member invite event](/v1.18/server-server-api/#inviting-to-a-room). This is the
simplest case.
...
homeserver will request the identity server to store an invite[store an invite](/v1.18/identity-service-api/#invitation-storage)
for this identifier and to deliver it to whoever binds it to its Matrix ID. It
will also send an `m.room.third_party_invite`[m.room.third_party_invite](/v1.18/client-server-api/#mroomthird_party_invite)
...
the identity server will send a request to the `/3pid/onbind`[/3pid/onbind](/v1.18/server-server-api/#put_matrixfederationv13pidonbind)
endpoint of the the ID’s homeserver as described in the Invitation
Storage[Invitation
Storage](/v1.18/identity-service-api/#invitation-storage) section of the Identity
Service API.

The following process applies for each invite sent by the identity
server:

The invited homeserver will create an `m.room.member`[m.room.member](/v1.18/client-server-api/#mroommember)
...
at the `/exchange_third_party_invite`[/exchange_third_party_invite](/v1.18/server-server-api/#put_matrixfederationv1exchange_third_party_inviteroomid)
endpoint.
...
| `mxid` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The user that is now bound to the third-party identifier. |
...
| `mxid` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The now-bound user ID that received the invite. |
| `room_id` | `Room ID[Room ID](/v1.18/appendices#room-ids)` | **Required: **The room ID the invite is valid for. |
| `sender` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The user ID that sent the invite. |
...
SignedThirdPartyInvite
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `mxid` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The user ID that has been bound to the third-party identifier. |
| `signatures` | `{string: {string: string}}` | **Required: **The identity server signatures for this block. This is a map of identity server name to signing key identifier to base64-encoded signature. The signatures are calculated using the process described at Signing JSON[Signing JSON](/v1.18/appendices/#signing-json). |
| `token` | `string` | **Required: **The token generated by the identity server at the `/store_invite`[/store_invite](/v1.18/identity-service-api/#post_matrixidentityv2store-invite) endpoint. It matches the `state_key` of the corresponding `m.room.third_party_invite`[m.room.third_party_invite](/v1.18/client-server-api/#mroomthird_party_invite) event. |
...
an invite as per the Inviting to a room[Inviting to a room](/v1.18/server-server-api/#inviting-to-a-room) section before returning a
...
path parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `roomId` | `Room ID[Room ID](/v1.18/appendices#room-ids)` | **Required: **The room ID to exchange a third-party invite in |
...
| `sender` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The user ID of the user who sent the original `m.room.third_party_invite` event. |
| `state_key` | `User ID[User ID](/v1.18/appendices#user-identifiers)` | **Required: **The user ID of the invited user |
...
To complement the room directory in the Client-Server API[room directory in the Client-Server API](/v1.18/client-server-api/#published-room-directory),
...
| `third_party_instance_id` | `string` | The specific third-party network/protocol to request from the homeserver. Can only be used if `include_all_networks` is false. This is the `instance_id` of a `Protocol Instance` returned by `GET /_matrix/client/v3/thirdparty/protocols`[GET /_matrix/client/v3/thirdparty/protocols](/v1.18/client-server-api/#get_matrixclientv3thirdpartyprotocols). |
...
| `canonical_alias` | `Room Alias[Room Alias](/v1.18/appendices#room-aliases)` | The canonical alias of the room, if any. |
...
| `room_id` | `Room ID[Room ID](/v1.18/appendices#room-ids)` | **Required: **The ID of the room. |
| `room_type` | `string` | The `type` of room (from `m.room.create`[m.room.create](/v1.18/client-server-api/#mroomcreate)), if any. |
| `topic` | `string` | The plain text topic of the room. Omitted if no `text/plain` mimetype exists in `m.room.topic`[m.room.topic](/v1.18/client-server-api/#mroomtopic). |
...
This API returns paginated responses. The rooms are ordered by the number
of joined members, with the largest rooms first.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
| `room_types` | `[string\|null]` | An optional list of room types[room types](/v1.18/client-server-api/#types) to search for. To include rooms without a room type, specify `null` within this list. When not specified, all applicable rooms (regardless of type) are returned. **Added in `v1.4`** |
...
To complement the Client-Server API’s Spaces module[Client-Server API’s Spaces module](/v1.18/client-server-api/#spaces),
...
Federation version of the Client-Server `GET /hierarchy`[GET /hierarchy](/v1.18/client-server-api/#get_matrixclientv1roomsroomidhierarchy)
...
Only `m.space.child`[m.space.child](/v1.18/client-server-api/#mspacechild) state events of the room are considered. Invalid child
...
query parameters
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `suggested_only` | `boolean` | Optional (default `false`) flag to indicate whether or not the server should only consider suggested rooms. Suggested rooms are annotated in their `m.space.child`[m.space.child](/v1.18/client-server-api/#mspacechild) event contents. |
...
SpaceHierarchyChildRoomsChunk
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `allowed_room_ids` | `[Room ID[Room ID](/v1.18/appendices#room-ids)]` | If the room is a restricted room[restricted room](/v1.18/server-server-api/#restricted-rooms), these are the room IDs which are specified by the join rules. Empty or omitted otherwise. |
| `avatar_url` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | The URL for the room’s avatar, if one is set. |
| `canonical_alias` | `Room Alias[Room Alias](/v1.18/appendices#room-aliases)` | The canonical alias of the room, if any. |
...
| `num_joined_members` | `integer` | **Required: **The number of members joined to the room. |
| `room_id` | `Room ID[Room ID](/v1.18/appendices#room-ids)` | **Required: **The ID of the room. |
| `room_type` | `string` | The `type` of room (from `m.room.create`[m.room.create](/v1.18/client-server-api/#mroomcreate)), if any. **Added in `v1.4`** |
...
| `avatar_url` | `URI[URI](https://datatracker.ietf.org/doc/html/rfc3986)` | The URL for the room’s avatar, if one is set. |
| `canonical_alias` | `Room Alias[Room Alias](/v1.18/appendices#room-aliases)` | The canonical alias of the room, if any. |
| `children_state` | `[StrippedStateEventStrippedStateEvent]` | **Required: **The `m.space.child`[m.space.child](/v1.18/client-server-api/#mspacechild) events of the space-room, represented as Stripped State Events[Stripped State Events](/v1.18/client-server-api/#stripped-state) with an added `origin_server_ts` key. If the room is not a space-room, this should be empty. |
| `encryption` | `string` | The encryption algorithm to be used to encrypt messages sent in the room. One of: `[m.megolm.v1.aes-sha2]`. **Added in `v1.15`** |
| `guest_can_join` | `boolean` | **Required: **Whether guest users may join the room and participate in it. If they can, they will be subject to ordinary power level rules like any other user. |
| `join_rule` | `string` | The room’s join rule. When not present, the room is assumed to be `public`. |
| `name` | `string` | The name of the room, if any. |
...
m.receipt
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `content` | `{Room ID[Room ID](/v1.18/appendices#room-ids): Room ReceiptsRoom Receipts}` | **Required: **Receipts for a particular room. The string key is the room ID for which the receipts under it belong. |
...
Room Receipts
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `m.read` | `{User ID[User ID](/v1.18/appendices#user-identifiers): User Read ReceiptUser Read Receipt}` | **Required: **Read receipts for users in the room. The string key is the user ID the receipt belongs to. |
...
that belong to the target server (identified by the server name[server name](/v1.18/appendices/#server-name)
in the alias).
...
to the target server (identified by the server name[server name](/v1.18/appendices/#server-name)
in the user ID).
...
error code of `M_FORBIDDEN`.| Rate-limited: | No |
| --- | --- |
| Requires authentication: | Yes |
...
module](/v1.18/client-server-api/#device-management)
...
| `signatures` | `{User ID[User ID](/v1.18/appendices#user-identifiers): {string: string}}` | **Required: **Signatures for the device key object. A map from user ID, to a map from `<algorithm>:<device_id>` to the signature. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.18/appendices/#signing-json). |
...
| `signatures` | `Signatures` | Signatures of the key, calculated using the process described at Signing JSON[Signing JSON](/v1.18/appendices/#signing-json). Optional for the master signing key. Other keys must be signed by the user's master signing key. |
...
module](/v1.18/client-server-api/#end-to-end-encryption)
...
/keys/upload[/keys/upload](/v1.18/client-server-api/#post_matrixclientv3keysupload). (All
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `one_time_keys` | `{User ID[User ID](/v1.18/appendices#user-identifiers): {string: string}}` | **Required: **The keys to be claimed. A map from user ID, to a map from device ID to algorithm name. Requested users must be local to the receiving homeserver. |
...
### 200 response| Name | Type | Description |
| --- | --- | --- |
| `one_time_keys` | `{User ID[User ID](/v1.18/appendices#user-identifiers): {string: {string: string\|KeyObjectKeyObject}}}` | **Required: **One-time keys for the queried devices. A map from user ID, to a map from devices to a map from `<algorithm>:<key_id>` to the key object. See the Client-Server Key Algorithms[Client-Server Key Algorithms](/v1.18/client-server-api/#key-algorithms) section for more information on the Key Object format. |
...
| `signatures` | `{string: {string: string}}` | **Required: **Signature of the key object. The signature is calculated using the process described at Signing JSON[Signing JSON](/v1.18/appendices/#signing-json). |
...
### Request body| Name | Type | Description |
| --- | --- | --- |
| `device_keys` | `{User ID[User ID](/v1.18/appendices#user-identifiers): [string]}` | **Required: **The keys to be downloaded. A map from user ID, to a list of device IDs, or to an empty list to indicate all devices for the corresponding user. Requested users must be local to the receiving homeserver. |
...
| `messages` | `{User ID[User ID](/v1.18/appendices#user-identifiers): {string: Device Message Contents}}` | **Required: **The contents of the messages to be sent. These are arranged in a map of user IDs to a map of device IDs to message bodies. The device ID may also be `*`, meaning all known devices for the user. |
...
API[Client-Server
API](/v1.18/client-server-api/#content-repository). When a server wishes
to serve content originating from a remote server, it needs to ask the
remote server for the media.

Servers MUST use the server described in the Matrix Content URI[Matrix Content URI](/v1.18/client-server-api/#matrix-content-mxc-uris).
...
endpoints described by the Content Repository module in the Client-Server API[Content Repository module in the Client-Server API](/v1.18/client-server-api/#content-repository),
...
| `504` | The content is not yet available. A standard error response[standard error response](/v1.18/client-server-api/#standard-error-response) will be returned with the `errcode` `M_NOT_YET_UPLOADED`. |
...
See the Client-Server API Thumbnails[Client-Server API Thumbnails](/v1.18/client-server-api/#thumbnails)
...
| `method` | `string` | The desired resizing method. See the Client-Server API Thumbnails[Client-Server API Thumbnails](/v1.18/client-server-api/#thumbnails) section for more information. One of: `[crop, scale]`. |
| `timeout_ms` | `integer` | The maximum number of milliseconds that the client is willing to wait to start receiving data, in the case that the content has not yet been uploaded. The default value is 20000 (20 seconds). The content repository SHOULD impose a maximum value for this parameter. The content repository MAY respond before the timeout. **Added in `v1.7`** |
...
ACLs[Server
ACLs](/v1.18/client-server-api/#server-access-control-lists-acls-for-rooms)
...
Additionally the `/_matrix/federation/v1/send/{txnId}`[/_matrix/federation/v1/send/{txnId}](/v1.18/server-server-api/#put_matrixfederationv1sendtxnid)
...
ACLs MUST be applied to all EDUs that are local to a specific room:
  ...
  - For typing notifications (`m.typing`)[typing notifications (m.typing)](/v1.18/server-server-api/#typing-notifications), if
the sending server is denied access to the room identified by
`room_id`, the EDU MUST be ignored.

  - For receipts (`m.receipt`)[receipts (m.receipt)](/v1.18/server-server-api/#receipts), all receipts for a particular
...
- `/_matrix/policy/v1/sign`[/_matrix/policy/v1/sign](/v1.18/server-server-api/#post_matrixpolicyv1sign) - **[Added in `v1.18`]**
...
- Normal server name resolution[Normal server name resolution](/v1.18/server-server-api/#resolving-server-names).

- Publishing a signing key[Publishing a signing key](/v1.18/server-server-api/#publishing-keys).

- Request authentication[Request authentication](/v1.18/server-server-api/#authentication).

- Being able to make and send join requests[make and send join requests](/v1.18/server-server-api/#joining-rooms).

- Receiving and processing `POST /_matrix/policy/v1/sign`[POST /_matrix/policy/v1/sign](/v1.18/server-server-api/#post_matrixpolicyv1sign)
requests.
...
- Receiving invites[invites](/v1.18/server-server-api/#inviting-to-a-room) can make it easier to add the Policy
Server to a room.

- Receiving transactions[transactions](/v1.18/server-server-api/#transactions) is recommended to avoid remote servers
flagging the Policy Server as “offline”, even if the contents are discarded.

- Receiving device lookups[device lookups](/v1.18/server-server-api/#get_matrixfederationv1userdevicesuserid) can also
...
- The *current state* for the room has a valid `m.room.policy`[m.room.policy](/v1.18/client-server-api/#mroompolicy)
...
Policy Server](/v1.18/server-server-api/#asking-for-a-policy-server-signature-on-an-event) for a signature
before sending an event. If the Policy Server refuses to sign the event being
sent, servers SHOULD fail to send that event as per the validation rules[validation rules](/v1.18/server-server-api/#validating-policy-server-signatures).

“Current state” shares the same definition as soft failure[soft failure](/v1.18/server-server-api/#soft-failure).
...
soft fail[soft fail](/v1.18/server-server-api/#soft-failure) or refuse to actually go forward with sending an event.
...
Because the Policy Server is not asked to sign `m.room.policy`[m.room.policy](/v1.18/client-server-api/#mroompolicy)
...
alongside the normal event signatures[event signatures](/v1.18/server-server-api/#signing-events), though uses a public
...
The Policy Server’s published signing key[published signing key](/v1.18/server-server-api/#publishing-keys) SHOULD NOT be the
...
ask for a new signature[ask for a new signature](/v1.18/server-server-api/#asking-for-a-policy-server-signature-on-an-event). If
...
event anyway, soft fail[soft fail](/v1.18/server-server-api/#soft-failure) the event.
...
Per above[above](/v1.18/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room), if a room has
...
After asking for a signature, homeservers SHOULD validate[validate](/v1.18/server-server-api/#validating-policy-server-signatures)
...
other requirements[other requirements](/v1.18/server-server-api/#policy-servers). This endpoint
is otherwise optional. See Unsupported Endpoints[Unsupported Endpoints](/v1.18/server-server-api/#unsupported-endpoints)
...
the room has enabled a Policy Server[enabled a Policy Server](/v1.18/server-server-api/#determining-if-a-policy-server-is-enabled-in-a-room).
...
implementation detail.| Rate-limited: | Yes |
| --- | --- |
| Requires authentication: | Yes |
...
| `403` | The calling server is ACL’d[ACL’d](/v1.18/server-server-api/#server-access-control-lists-acls). This endpoint is optionally affected by server ACLs. The Policy Server MAY NOT apply an ACL to the request, especially if the Policy Server is not tracking the room DAG. |
| `404` | The server is not a Policy Server, or the room is unknown/not protected by the Policy Server. Error codes include: - `M_NOT_FOUND` - The room is not known or is not protected by the Policy Server. - `M_UNRECOGNIZED` - The server is not a Policy Server. See Unsupported Endpoints[Unsupported Endpoints](/v1.18/server-server-api/#unsupported-endpoints) for more detail. |
| `429` | This request was rate-limited. |
...
Base64[Unpadded
Base64](/v1.18/appendices/#unpadded-base64) and stored in the event
object, in a `hashes` object, under a `sha256` key.

The event object is then *redacted*, following the redaction
algorithm[redaction
algorithm](/v1.18/client-server-api/#redactions).
Finally it is signed as described in Signing
JSON[Signing
JSON](/v1.18/appendices/#signing-json), using the server’s signing key
(see also Retrieving server keys[Retrieving server keys](/v1.18/server-server-api/#retrieving-server-keys)).
...
**[Added in `v1.18`]**
Events sent in rooms with Policy Servers[Policy Servers](/v1.18/server-server-api/#policy-servers) MUST ask[ask](/v1.18/server-server-api/#asking-for-a-policy-server-signature-on-an-event)
...
redaction
algorithm[redaction
algorithm](/v1.18/client-server-api/#redactions), and
...
signature](/v1.18/appendices/#checking-for-a-signature). Note that this
...
**[Added in `v1.18`]**
Events sent in rooms with Policy Servers[Policy Servers](/v1.18/server-server-api/#policy-servers) have additional[additional](/v1.18/server-server-api/#validating-policy-server-signatures)
...
specification[room version
specification](/v1.18/rooms/) for more information. It is
...
JSON[Canonical
JSON](/v1.18/appendices/#canonical-json).
...
JSON[Canonical
JSON](/v1.18/appendices/#canonical-json), and the JSON is hashed using
SHA-256.
