...
Module[Push
Module](/v1.18/client-server-api/#push-notifications).
...
`/_matrix/client/v3/pushers/set`[/_matrix/client/v3/pushers/set](/v1.18/client-server-api/#post_matrixclientv3pushersset).| Rate-limited: | No |
| --- | --- |
...
| --- | --- | --- |
| `notification` | `NotificationNotification` | **Required: **Information about the push notification |
...
| `counts` | `CountsCounts` | This is a dictionary of the current number of unacknowledged communications for the recipient user. Counts whose value is zero should be omitted. |
| `devices` | `[DeviceDevice]` | **Required: **This is an array of devices that the notification should be sent to. |
...
| `prio` | `string` | The priority of the notification. If omitted, `high` is assumed. This may be used by push gateways to deliver less time-sensitive notifications in a way that will preserve battery power on mobile devices. One of: `[high, low]`. |
...
| `data` | `PusherDataPusherData` | A dictionary of additional pusher-specific data. This is the `data` dictionary passed in at pusher creation[pusher creation](/v1.18/client-server-api/#post_matrixclientv3pushersset) minus the `url` key. |
...
PusherData
 ...
 | Name | Type | Description |
| --- | --- | --- |
| `format` | `string` | The format to use for sending notifications. |
