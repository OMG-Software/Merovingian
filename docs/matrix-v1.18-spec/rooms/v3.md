...
This room version builds on version 2[version 2](/v1.18/rooms/v2/) with an improved event
format.
...
redaction algorithm locally should refer to the redactions[redactions](/v1.18/rooms/v3/#redactions)
...
all the remaining behaviour described by room version 2[room version 2](/v1.18/rooms/v2/).
...
Base64[Unpadded
Base64](/v1.18/appendices/#unpadded-base64), prefixed with `$`. A
...
`GET /_matrix/federation/v1/make_join/{roomId}/{userId}`[GET /_matrix/federation/v1/make_join/{roomId}/{userId}](/v1.18/server-server-api/#get_matrixfederationv1make_joinroomiduserid).)
...
the Handling Redactions[Handling Redactions](/v1.18/rooms/v3/#handling-redactions) section.
