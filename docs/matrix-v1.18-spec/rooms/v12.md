...
This room version builds on version 11[version 11](/v1.18/rooms/v11/), iterating on the state resolution
...
`m.room.create` event, per below. See the room ID grammar[room ID grammar](/v1.18/appendices/#room-ids)
...
the Handling redactions[Handling redactions](/v1.18/rooms/v12/#handling-redactions) section.
...
is not an array of strings where each string passes the same user ID[user ID](/v1.18/appendices/#user-identifiers)
...
don’t match those specified by the auth events selection[auth events selection](/v1.18/server-server-api/#auth-events-selection)
...
algorithm found in room version 2[room version 2](/v1.18/rooms/v2/) with the following modifications:


1. 


The *iterative auth checks algorithm* in the Algorithm[Algorithm](/v1.18/rooms/v12/#algorithm) subsection
...
A new definition[definition](/v1.18/rooms/v12/#definitions) for *conflicted state subgraph* has been added
...
- `m.room.member`[m.room.member](/v1.18/client-server-api/#mroommember) allows keys `membership`, `join_authorised_via_users_server`.
Additionally, it allows the `signed` key of the `third_party_invite` key.

- `m.room.create`[m.room.create](/v1.18/client-server-api/#mroomcreate) allows all keys.

- `m.room.join_rules`[m.room.join_rules](/v1.18/client-server-api/#mroomjoin_rules) allows keys `join_rule`, `allow`.
...
- `m.room.history_visibility`[m.room.history_visibility](/v1.18/client-server-api/#mroomhistory_visibility)
allows key `history_visibility`.

- `m.room.redaction`[m.room.redaction](/v1.18/client-server-api/#mroomredaction) allows key `redacts`.


### Handling redactions

In room versions 1 and 2, redactions were
explicitly part of the authorization rules[authorization rules](/v1.18/rooms/v1/#authorization-rules)
under Rule 11. As of room version 3, these conditions no longer exist as
represented by this version’s authorization rulesthis version’s authorization rules.
...
hash[reference
hash](/v1.18/server-server-api/#calculating-the-reference-hash-for-an-event) of
the event encoded using a variation of Unpadded
Base64[Unpadded
Base64](/v1.18/appendices/#unpadded-base64) which replaces the 62nd and
63rd characters with `-` and `_` instead of using `+` and `/`. This
matches RFC4648’s definition of URL-safe
base64[RFC4648’s definition of URL-safe
...
appendices[appendices](/v1.18/appendices/#canonical-json). This translates to a
...
/_matrix/key/v2/server](/v1.18/server-server-api/#get_matrixkeyv2server)
or POST
/_matrix/key/v2/query[POST
/_matrix/key/v2/query](/v1.18/server-server-api/#post_matrixkeyv2query)
