...
bytes using Canonical JSON[Canonical JSON](/v1.18/appendices/#canonical-json), computing the signature
...
Adapted from the grammar in http://tools.ietf.org/html/rfc7159[http://tools.ietf.org/html/rfc7159](http://tools.ietf.org/html/rfc7159)
...
signature is encoded using unpadded Base64[unpadded Base64](/v1.18/appendices/#unpadded-base64). The resulting base64
...
JSON[Canonical
JSON](/v1.18/appendices/#canonical-json) encoding.
...
the room versions specification[room versions specification](/v1.18/rooms/) for more
information.
...
Event IDs[Event IDs](/v1.18/appendices/#event-ids) section below for more information.
...
The `domain` of a user ID is the server name[server name](/v1.18/appendices/#server-name) of the
...
However, the precise format depends upon the room version specification[room version specification](/v1.18/rooms/):
...
(`A-Z`, `a-z`, `0-9`) when generating them. The `domain` is the server name[server name](/v1.18/appendices/#server-name)
...
event ID[event ID](/v1.18/appendices/#event-ids) of the `m.room.create` event using `!` as the sigil instead
...
parse the user IDs[user IDs](/v1.18/appendices/#user-identifiers) found in the `m.room.create` event’s `sender`.
...
The `domain` of a room alias is the server name[server name](/v1.18/appendices/#server-name) of the
...
specification](/v1.18/rooms/): early room versions included a `domain` component,
...
Common Identifier Format[Common Identifier Format](/v1.18/appendices/#common-identifier-format).
...
routing Room IDs is described below[below](/v1.18/appendices/#routing), and is encouraged for use in
...
Common Namespaced Identifier format[Common Namespaced Identifier format](/v1.18/appendices/#common-namespaced-identifier-grammar) and
...
versions[room
versions](/v1.18/rooms/). These slashes should normally be
...
the specification and is subsequently replaced with Spaces[Spaces](/v1.18/client-server-api/#spaces). Historical
...
Client Server `/join/{roomIdOrAlias}` API[Client Server /join/{roomIdOrAlias} API](/v1.18/client-server-api/#post_matrixclientv3joinroomidoralias).
