...
| Authentication and sessions | `runtime-wired` | LibSodium password hashing, CSPRNG access tokens, durable token hashes, SQLite/PostgreSQL hydration into runtime sessions, token-file-enforced public registration, explicit admin bootstrap API, client-server register/login/logout/whoami/device routes, policy checks, durable audit events, and restart-survival coverage | Add refresh-token rotation, operator bootstrap CLI controls, account recovery controls, global logout, and Matrix conformance fixtures. |
