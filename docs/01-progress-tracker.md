...
## 0.5.6 (in progress â€” codex/fix-e2ee-bootstrap-key-query)
...
## 0.5.5 (in progress â€” codex/fix-messages-eventid-and-preflight-rate-limit)
...
## 0.5.0 (in progress â€” feature/0.5.0-rate-limit-and-logging-config)
...
## 0.4.62 (in progress â€” fix/otk-signature-claim)
...
_None â€” all Alpha gates are closed. The alpha release itself remains a
...
- `GET /rooms/{roomId}/state/{eventType}/{stateKey}` implemented inline in
  `client_server.cpp`: looks up `PersistentStateEvent` â†’ `PersistentEvent`,
...
- E2EE round-trip conformance tests added: `keys/upload` â†’ `keys/query` round-
  trip, `keys/upload` â†’ `keys/claim` OTK consumption, `sendToDevice` â†’ sync
...
- `.clangd` tab indentation fixed â€” YAML spec prohibits tabs; spaces now used
...
- Sync `next_batch` off-by-one fix (0.4.27): The `next_batch` token in `/sync`
  ...
  already applied the `- 1U` correction â€” only the token construction was
  missing it.
...
- Federation signing conformance tests (0.4.38): spec-vectored unit tests
  ...
  vectors (key sorting, Unicode normalisation, `-0`â†’`0`, integer-only
...
- Remote join content hash (0.4.25): the `make_join` â†’ `send_join` remote
...
- Fail-closed alpha hardening controls. `HardeningStatus::alpha_exception`
  ...
  prints a `production_ready=â€¦/alpha_ready=â€¦` readiness summary. The
...
- Login `device_id` collision fix (0.4.61).
  ...
  Matrix v1.18 Â§5.3.2 requires the server to mint a unique opaque
...
- Matrix UI-Interactive Authentication for `POST /register`: absent `auth` field
  ...
  `auth.type`) also returns 401 with the UIA challenge â€” not 403.
...
- Feature (0.4.4): Outbound membership wired into `join_room` for remote rooms.
  ...
  `make_join` â†’ sign â†’ `send_join` flow with the remote homeserver.
...
- Infra (0.4.52): Added a ThreadSanitizer `tsan` job to
  ...
  sanitizer job ran only ASan+UBSan, which cannot detect data races â€” the reason
...
- Fix (0.4.55): federated `GET /_matrix/federation/v1/query/profile` now
  ...
  as absent.
- Fix (0.4.51): `m.receipt` federation EDU content format â€” the receipt content
...
- Fix (0.4.50): Key backup routing â€” `PUT /room_keys/keys?version=N` returned 404
...
- Fix (0.4.28): Inbound `send_join` (v2) response was missing the `"event"`
  field. Per Matrix federation spec Â§11.5.1 the resident server must echo the
...
- Fix (0.4.8): `SocketHandle` fd ownership transferred incorrectly to pool
  workers â€” `native_handle()` followed by reset closed the fd before the
...
- Federation request signing follows the Matrix-spec X-Matrix scheme: the
  ...
  shared-secret `verify_token` derivation â€” which could not interoperate with
  other homeservers â€” has been removed. PDU event signatures are verified via
...
1. Complete Matrix v1.18 conformance for client-server endpoints currently
   marked `partial` â€” promote them to `covered` with conformance fixtures.
2. Add Matrix federation conformance fixtures covering join, leave, invite,
   backfill, and key-rotation scenarios end to end.
3. Retire one hardening alpha exception per minor release â€” start with the
...
- `send_join` response now returns room state **prior to** the new join event,
  as required by spec Â§11.5.1. Previously the join event was persisted before
  ...
  recalculate expected `auth_events` â€” finding the join event itself as the
