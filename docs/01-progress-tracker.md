...
- Extend the beta Matrix v1.18 client-server conformance fixture beyond the
  covered authentication, devices, rooms, sync, media, reports, and E2EE key
  happy paths into negative cases, UI-auth fallback flows, and account recovery.
...
| Profile | `GET /_matrix/client/v3/profile/{userId}` | `partial` | Returns `{"displayname": "", "avatar_url": ""}` stub and is covered by the v1.18 fixture. Cinny fetches this after login to populate the user-info header. Needs real displayname/avatar storage and PUT to update them. |
