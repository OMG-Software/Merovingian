...
Added 12 new conformance scenarios for client-server API endpoints that had no
spec-conformance test coverage. All 47 tests pass.

| Endpoint | Spec section | Gap filled |
|----------|-------------|------------|
| GET /devices | §devices | devices array, device_id present |
| GET /devices/{deviceId} | §devices | 200 known, 404 unknown |
| GET /capabilities | §capabilities | m.room_versions default+available |
| GET /joined_rooms | §joined-rooms | joined_rooms array |
| GET /publicRooms | §public-rooms | chunk + total_room_count_estimate |
| GET /directory/room/{alias} | §room-aliases-and-canonical-aliases | room_id+servers, 404 unknown |
| GET /register/available | §username-availability | available, M_USER_IN_USE, M_INVALID_USERNAME |
| PUT+GET /user/{userId}/account_data/{type} | §account-data | store+retrieve, 404 unset |
| PUT+GET /profile/{userId}/displayname | §profiles | update+reflect |
| GET /profile/{userId} | §profiles | unauthenticated, displayname+avatar_url, 404 unknown |
| GET /pushrules/ | §push-rules | global with all five categories |
| Standard errors | §standard-error-response | errcode+error on 401 and 403 |
...
### Conformance coverage gaps (spec fixes + new tests)

Added conformance test coverage across sync filter, redaction, event auth rules,
and state resolution; fixed three implementation bugs exposed by the new tests.

**Redaction fixes:**

- `m.room.join_rules`: `allow` key was unconditionally preserved; now only
  preserved in v11+ (`RedactionRules::room_v11_plus`). Spec: room v11 redactions.
- `m.room.aliases`: no case existed — all versions stripped `aliases`.
  Fixed: `aliases` preserved in v1-v10, stripped in v11+.
- `m.room.third_party_invite`: `signed` key now preserved in all versions
  (case added in previous session; confirmed passing).

**Event auth fix:**

- Added `MembershipState::knock` to the enum (was missing; `parse_membership_state`
  fell through to `leave`, making knock events look like self-leave).
- `parse_membership_state("knock")` now returns `MembershipState::knock`.
- Added step-5 knock block in `authorize_event_against_auth_events`:
  sender must equal state_key, sender must not be banned/joined/invited,
  and join_rule must be `knock` or `knock_restricted`.

**New conformance test scenarios (37 new SCENARIO blocks):**

- `test_sync_filter_conformance.cpp`: rooms/not_rooms, state/ephemeral/
  account_data subfilters, presence filter, global account_data, limit,
  include_leave.
- `test_redaction_conformance.cpp`: join_rules v10 vs v11+, history_visibility,
  aliases v10 vs v11+, third_party_invite signed, state_key top-level,
  v12 inherits v11 rules.
- `test_event_auth_rules.cpp`: non-create without create event, knock
  allow/reject, kick at equal PL, ban at equal PL.
- `test_state_resolution_conformance.cpp`: v2 conflicting membership,
  v2 unconflicted + conflicted, partition_conflicted_state.

#### Files changed

| File | Change |
|------|--------|
| `include/merovingian/events/authorization.hpp` | Add `MembershipState::knock` to enum |
| `src/events/authorization.cpp` | Add `knock` to `membership_name`, `parse_membership_state`, and step-5 block |
| `src/events/redaction.cpp` | Fix join_rules `allow` v11+ only; add aliases v1-v10 case |
| `tests/conformance/test_sync_filter_conformance.cpp` | 6 new scenarios |
| `tests/conformance/test_redaction_conformance.cpp` | 7 new scenarios |
| `tests/conformance/test_event_auth_rules.cpp` | 5 new scenarios |
| `tests/conformance/test_state_resolution_conformance.cpp` | 3 new scenarios |

## 0.5.12 (merged — #204)

### Canonical JSON multi-byte key sort test (spec conformance fix)

The scenario “Canonical JSON sorts object keys by Unicode code point (byte order)”
in `tests/conformance/test_canonicaljson_serializer.cpp` claimed to test
multi-byte UTF-8 keys but used only ASCII keys (`{"z":3,"A":1,"a":2}`).
Replaced with a vector that includes `é` (U+00E9, bytes `0xC3 0xA9`) and
`中` (U+4E2D, bytes `0xE4 0xB8 0xAD`) as actual object keys, verifying
byte-lexicographic ordering places multi-byte keys after all ASCII keys.

### Sync filter helper comment (accuracy fix)

The `[helper]`-tagged scenario in `tests/conformance/test_sync_filter_conformance.cpp`
had a comment labelled “Spec behaviour” claiming that invalid filter input must
result in an unrestricted pass-all filter. This is implementation helper behaviour,
not Matrix API behaviour — the spec requires `400 M_BAD_JSON`. Comment updated.

#### Files changed

| File | Change |
|------|--------|
| `tests/conformance/test_canonicaljson_serializer.cpp` | Replace ASCII-only key sort vector with `é`/`中` multi-byte keys |
| `tests/conformance/test_sync_filter_conformance.cpp` | Correct “Spec behaviour” comment to “Implementation helper behaviour” |

## 0.5.11 (merged — #203)

### User ID localpart validator split (spec conformance fix)

The single `localpart_is_valid()` helper was inconsistent: its comments claimed
it accepted "the full historical set for federation compatibility" but it actually
rejected `#`, Unicode, and empty localparts that historical deployments issued.
It also accepted uppercase, which is only valid for historical IDs — not new ones.

**Changes:**

- `localpart_is_valid()` removed; replaced by two spec-faithful validators:
  - `localpart_is_valid_new()` — normative Matrix v1.18 set: `a-z`, `0-9`,
    `._-=/+`. Rejects uppercase, empty, spaces, `#`, and all other characters.
    Used at registration and all local auth paths.
  - `localpart_is_valid_federated()` — historical set: any valid UTF-8 code
    point except `:` (breaks parsing) and NUL (corrupts storage). Surrogate
    code points (U+D800–U+DFFF) are rejected. Empty localparts accepted for
    maximum federation compatibility.
- `user_id_is_valid()` updated to call `localpart_is_valid_new()`.
  `@Alice:example.org` is now correctly rejected by the strict path.
- `user_id_is_valid_federated()` added — same structural rules (`@`, `:`,
  255-byte limit, valid server name) but uses the federated localpart validator.
  Allows empty localparts (separator at position 1).
- Both callers of `localpart_is_valid()` in `client_server.cpp` (registration
  paths) updated to `localpart_is_valid_new()`.
- `test_identifier_grammar.cpp` updated:
  - `localpart_is_valid_new()` scenarios: normative acceptance, uppercase
    rejection, forbidden-character rejection.
  - `localpart_is_valid_federated()` scenarios: historical characters (`#`,
    `!`, `~`, Unicode), empty localpart, rejection of `:` and NUL, rejection
    of invalid UTF-8 and surrogate code points.
  - `user_id_is_valid_federated()` scenario: historical user IDs accepted,
    structurally broken user IDs still rejected.
- `test_auth.cpp` unit test corrected: previously asserted that
  `user_id_is_valid("@Alice:example.org")` returns `true`; now correctly
  asserts `false` (strict path) and shows the federated path accepts it.

## 0.5.10 (in progress — fix/codex-conformance-audit)

### Spec conformance test infrastructure overhaul

- Created `tests/conformance/` directory to hold all Matrix v1.18 strict
  spec conformance tests, distinct from unit and integration tests.
- Moved 10 existing spec conformance test files from `tests/unit/` to
  `tests/conformance/`: `test_canonicaljson_parser.cpp`,
  `test_canonicaljson_serializer.cpp`, `test_client_server_conformance.cpp`,
  `test_event_auth_rules.cpp`, `test_event_authorization.cpp`,
  `test_events.cpp`, `test_federation_conformance.cpp`,
  `test_key_publication_conformance.cpp`, `test_server_discovery.cpp`,
  `test_x_matrix_auth_parsing.cpp`.
- Added `tests/conformance/CLAUDE.md` documenting the conformance testing
  rules (never weaken assertions, always cite spec sections, spec is authority).
- Added a dedicated `merovingian-conformance-tests` meson executable and
  `conformance-tests` meson test target.
- Updated `scripts/check-unit-test-registration.sh` to also validate the
  `tests/conformance/` directory.
- Added new conformance test files covering previously untested spec areas:
  - `test_identifier_grammar.cpp` — user IDs, localparts, server names,
    event IDs per Matrix v1.18 Appendices § Identifier Grammar.
  - `test_signing_json_conformance.cpp` — signing JSON algorithm, payload
    field exclusion, round-trip sign/verify, multi-server signature
    preservation, presence checks.
- Strengthened `test_canonicaljson_parser.cpp`:
  - Added spec-mandated integer range tests: numbers outside
    `[-(2^53)+1, (2^53)-1]` MUST be rejected (identifies real bug).
  - Added all spec Example test vectors from the appendices.
  - Added test that ` ` in a JSON string round-trips correctly
    (identifies real parser bug — currently returns `unexpected_end`).
- Strengthened `test_canonicaljson_serializer.cpp`:
  - Added control character escaping tests (U+0000–U+001F → `\uXXXX`).
  - Added key-sorting by Unicode code point (uppercase before lowercase).
  - Added no-insignificant-whitespace check.

### Conformance test audit — Codex static-analysis findings (4 genuine bugs fixed)

Four genuine bugs were identified by Codex's static analysis of the conformance
suite against the v1.18 spec:

1. **`-0` canonical JSON** — `is_canonical_int64_token()` accepted `-0` (it
   matched the `^-?0$` shape and `from_chars` returned 0). The spec explicitly
   says "Negative zero MUST NOT appear in canonical JSON." Fixed the parser to
   return `invalid_number` for `-0`. Updated three test assertions that
   incorrectly expected `ParseError::none`.

2. **Uppercase localpart labelled "Spec MUST"** — `test_identifier_grammar.cpp`
   asserted `localpart_is_valid("ALICE")` and labelled it with "Spec MUST: a-z
   A-Z 0-9…". New v1.18 localparts MUST be lowercase; uppercase exists only in
   historical IDs. Split into a normative scenario (lowercase only, `[conformance]`)
   and a `[historical]` scenario noting federation compatibility.

3. **Server discovery "private" string** — REQUIRE checking that the rejection
   reason contains `"private"` was labelled "Spec MUST". The spec mandates
   `discovery_allowed == false` but says nothing about the error string. Changed
   to `REQUIRE_FALSE(result.reason.empty())`.

4. **Sync filter `[conformance]` tag** — `parse_filter_argument` invalid-JSON
   scenario tested internal helper leniency (pass-all on malformed input), not
   the Matrix API. Retagged `[helper]`.

### Codex conformance audit — remaining 6 findings fully investigated and fixed

After re-investigation, all six previously-dismissed findings were confirmed as
genuine spec violations or test correctness gaps. All fixed:

1. **`make_create_event()` fixture had `room_id` for v12** — Removed
   `"room_id":"!room:example.org"` from the v12 create event test fixture in
   `test_event_auth_rules.cpp`. Room Version 12 `m.room.create` events MUST
   NOT include a `room_id` field.

2. **`parse_event_envelope()` rejected v12 create events** — Real production
   bug: `event.cpp` line 143 required `room_id` for ALL events, causing v12
   create events to fail inbound parsing. Fixed to allow absent `room_id` when
   `type == "m.room.create"`. The `room_id` is now populated by the caller
   (`parse_inbound_pdu_envelope`) after computing the reference hash.

3. **`parse_inbound_pdu_envelope()` did not derive room_id for v12 creates** —
   Fixed to compute `room_id = "!" + event_id.substr(1)` for v12 create events
   (swapping the `$` sigil to `!`, same hash body). Added PDU format conformance
   test verifying v12 create events are accepted and room_id is correctly derived.

4. **v12 spec violation: create event in auth_events** — Added enforcement in
   `parse_inbound_pdu_envelope`: for v12 rooms (room_id has no `:` domain), the
   create event ID is computable as `$` + hash. If any entry in `auth_events`
   equals this ID, the PDU is rejected. Added conformance test for the rejection
   (bad PDU) and acceptance (well-formed PDU with other auth events).

5. **Event ID grammar/semantics distinction** — Added a new conformance scenario
   in `test_identifier_grammar.cpp` clarifying that `event_id_is_valid()` is a
   SYNTAX check only. Short IDs like `$abc123` are grammar-valid but cannot be
   real SHA-256 derived event IDs (which are always 44 chars: `$` + 43 base64url).

6. **Sync `/sync` returned no error for malformed inline filter** — Real API bug:
   the route handler silently degraded a `{invalid json}` filter to pass-all.
   Fixed in `client_server.cpp`: if `?filter=` starts with `{` and fails
   `canonicaljson::parse_lossless`, return `400 M_BAD_JSON`. Added conformance
   test in `test_sync_filter_conformance.cpp`.

### Default push ruleset fix — two missing mandatory override rules

- Added `.m.rule.contains_display_name` (condition kind `contains_display_name`)
  and `.m.rule.roomnotif` (matches `@room` in body with sender notification
  permission) to the global override ruleset returned by `GET /pushrules/`.
  Both are MUST per CS API v1.18 § Default Override Rules. Their absence was
  observed in a live Element rageshake log — the SDK logged “Missing default
  global override push rule” on every login and patched them in client-side.
- Added `push_condition_contains_display_name()` helper.
- Expanded `/pushrules/` conformance test to assert all twelve default override
  rules and all five underride rules are present.

### Third conformance pass â€” transaction envelope enforcement and request-auth fixes

- `PUT /_matrix/federation/v1/send/{txnId}` now enforces the Matrix v1.18
  transaction envelope instead of accepting legacy bare-PDU request bodies:
  `origin`, `origin_server_ts`, and `pdus` are required, `edus` must be an
  array when present, and `pdus: []` is accepted.
- Inbound federation request-signature mismatches and bad remote signatures now
  return `403` rather than `502`, aligning the runtime with auth-failure
  semantics expected by the strict conformance suite.
- Stale local tests were updated to reflect spec-correct uppercase localpart
  acceptance and deterministic basic state-resolution conflict handling.
- Two failing relationship conformance scenarios were corrected to send valid
  JSON for `m.replace` and `m.thread`; the implementation path itself was not
  the defect there.

### Known spec violations surfaced by new conformance tests

The following tests now FAIL, correctly identifying implementation bugs to fix:

1. **`test_canonicaljson_parser.cpp`** — Parser accepts integers outside
   `[-(2^53)+1, (2^53)-1]` (e.g. int64_max `9223372036854775807`). Spec MUST
   reject them. See: https://spec.matrix.org/v1.18/appendices/#grammar
2. **`test_canonicaljson_parser.cpp`** — Parser returns `unexpected_end` for
   `{"a":" "}`. The NUL-byte JSON escape must parse successfully.
3. **`test_identifier_grammar.cpp`** — `localpart_is_valid` rejects uppercase
   letters (e.g. `"ALICE"`). The spec grammar includes `ALPHA` (both cases)
   in the allowed localpart character set.

These tests are intentionally left failing until the implementation is fixed.

### Second conformance pass — accuracy fixes and new coverage

**Accuracy fixes applied:**
- `test_identifier_grammar.cpp`: Fixed user ID max-length test to exercise the exact 255-byte boundary (255 valid, 256 invalid) rather than non-boundary values.
- `test_signing_json_conformance.cpp`: Corrected pre-existing signature in "preserves multi-server signatures" test from 88 to 86 characters (= 64-byte Ed25519 signature in unpadded base64).
- `test_key_publication_conformance.cpp`: Added assertion that `valid_until_ts` is a positive integer and at least 60 seconds from the Unix epoch (catches a zero/epoch-relative regression).
- `test_redaction_conformance.cpp` (new): Tests exact field-level redaction behaviour per room version for v10 and v11 rules — covering preserved top-level fields, per-event-type content survival, and the v11 breaking changes (`origin` removal, full `m.room.create` content, `invite` in power levels, `redacts` under content).
- `test_signing_json_conformance.cpp` (new vectors): Added the four spec-exact cryptographic test vectors from https://spec.matrix.org/v1.18/appendices/#cryptographic-test-vectors — all four **pass**, confirming Ed25519 signing and standard-base64 encoding are correct.
- `test_event_auth_rules.cpp` (new): Added room-version-specific auth rule tests including v1 vs v6 sender-domain check, stable-version policy coverage, and v11/v12 support confirmation.

**New spec violations surfaced (6 total failures):**

The same 3 bugs from the first pass remain. Three new failures are newly found:

4. **`test_event_auth_rules.cpp:1049/1103`** — `find_room_version_policy("1")` returns null. Room versions 1–9 are not registered. The spec MUST: a compliant server supports all stable room versions.
5. **`test_event_auth_rules.cpp:1137`** — Same root cause: stable versions "1"–"9" have no registered policy.

These 5 tests correctly identify implementation gaps.

**Follow-up fix applied on this branch:**
- `src/rooms/room_version_policy.cpp` now registers all stable room versions
  `1` through `12`. Versions `1`–`5` use the original auth rules,
  versions `6`–`12` use the v6+ auth rules, version `1` keeps state
  resolution v1, versions `2`–`12` use state resolution v2, versions
  `1`–`2` keep the legacy event format, versions `3`–`12` use the modern
  event format, and version `11`/`12` keep the newer redaction rules.
