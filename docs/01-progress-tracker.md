...
| --- | --- | --- | --- |
| Authentication | `POST /_matrix/client/v3/register` | `partial` | Matrix JSON body is parsed, registration-token UI auth is enforced from the configured token file, public registration creates non-admin users, local registration is reachable through the client listener, and SQLite-backed local users survive restart. Fixed: INSERT SQL for device and access-token rows was missing parentheses (broke all registrations). Needs full UI auth flows, PostgreSQL coverage, and conformance fixtures. |
| Authentication | `POST /_matrix/client/v3/login` | `partial` | Password login works for local users with LibSodium-backed hashes, token hashes are SQLite-persisted, and restart-survival is tested. Fixed: INSERT SQL for device and access-token rows was missing parentheses (broke all logins). Needs full Matrix login flows, refresh behavior, PostgreSQL coverage, and conformance fixtures. |
...
| Sync | `POST /_matrix/client/v3/user/{userId}/filter` | `partial` | Filter upload is runtime-wired through the client-server adapter. The filter JSON is stored verbatim and an opaque `filter_id` is returned. Needs SQLite/PostgreSQL restart-survival coverage and conformance fixtures. |
| Sync | `GET /_matrix/client/v3/user/{userId}/filter/{filterId}` | `partial` | Filter retrieval is runtime-wired. Returns the stored filter JSON. Needs restart-survival coverage and conformance fixtures. |
| Account data | `PUT`/`GET /_matrix/client/v3/user/{userId}/account_data/{type}` | `partial` | Global (non-room) account data is runtime-wired through the persistent store with an upsert and surfaces in `/sync`. Cinny stores `m.direct` here. Needs room-scoped account data (`/rooms/{roomId}/account_data/{type}`) and conformance fixtures. |
...
| Media | `GET /_matrix/media/v3/config` | `partial` | Returns `{"m.upload.size": 104857600}` (100 MiB stub). Cinny fetches this after login to know the maximum attachment size. Needs wiring to the configured `security.media.max_upload_size` value. |
...
| E2EE keys | Device keys, one-time keys, fallback keys, cross-signing, backup APIs | `partial` | Authenticated key API route shapes are runtime-wired through the client-server adapter with durable server-blind key storage, one-time-key consumption, fallback-key reuse, backup rows, payload redaction, audit records, and SQLite restart coverage. The body limit for the client API has been raised from 4 KiB to 64 KiB so real OTK bundles (device keys + one-time keys) are accepted. Needs Matrix device-list stream semantics, complete backup retrieval/deletion semantics, full key-count behavior, and conformance fixtures. |
| Profile | `GET /_matrix/client/v3/profile/{userId}` | `partial` | Returns `{"displayname": "", "avatar_url": ""}` stub. Cinny fetches this after login to populate the user-info header. Needs real displayname/avatar storage and PUT to update them. |
...
| VoIP | `GET /_matrix/client/v3/voip/turnServer` | `partial` | Returns an empty object. No TURN server is configured; an empty 200 lets clients disable VoIP gracefully. Needs real TURN credential issuance once VoIP is supported. |
