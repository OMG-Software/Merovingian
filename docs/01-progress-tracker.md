...
| E2EE key APIs | `runtime-wired` | Key API route/planning boundary, authenticated client-server runtime dispatch for upload/query/claim/cross-signing/signature/backup route shapes, durable device/one-time/fallback/cross-signing/signature/backup storage, one-time-key consumption, fallback-key reuse, server-blind payload redaction, audit records, SQLite restart coverage, spec-correct `{"version":"1"}` response from `POST /room_keys/version` (0.4.31), working `DELETE /room_keys/version/{version}` removing the backup from the store and database (0.4.32), and conformance tests asserting delete+404 round-trip | Add Matrix device-list stream semantics, full key-count algorithms, complete backup session retrieval/deletion, Matrix v1.18 semantics, and remaining conformance fixtures. |
...
| E2EE keys | Device keys, one-time keys, fallback keys, cross-signing, backup APIs | `spec-covered` | Authenticated key API routes are runtime-wired through the client-server adapter with durable server-blind key storage, request-body-driven `/keys/query` and `/keys/claim`, one-time-key consumption, fallback-key reuse, `/keys/device_signing/upload`, `/keys/signatures/upload`, `/keys/changes`, `PUT /sendToDevice/{eventType}/{txnId}`, full backup version/batch/room/session get-put-delete coverage, payload redaction, audit records, SQLite restart coverage, 64 KiB body limit, and percent-decoded room-key backup path handling. v1.18 fixtures now cover upload/query/claim, cross-signing publication, uploaded-signature propagation, malformed upload/query/claim rejection, room-key backup metadata plus retrieval/update/delete semantics, federated inbound `m.direct_to_device` delivery into `/sync to_device.events`, and restart-safe outbound to-device federation dispatch that preserves the client txn ID as the EDU `message_id`. Needs Matrix device-list stream semantics and fuller one-time-key count behavior. |
