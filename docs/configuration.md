...
| `federation.worker.relay_threads` | unsigned int | `32` | Thread pool size for worker-handled endpoints that can block on a synchronous IPC round-trip back to main (PDU-bearing `send`, `send_join`/`send_leave`/`send_knock`, `invite`, `query/profile`, `user/keys/query`, `user/keys/claim`, `user/devices/{userId}`, `event/{eventId}`) or on outbound HTTP to a remote server. EDU-only `send` transactions are handled directly in main after X-Matrix verification so E2EE to-device key shares do not queue behind shard-0 worker traffic. Deliberately separate and generously sized from `threads`: these threads spend nearly all their time blocked on I/O rather than CPU, and sharing one small pool between both classes let a burst of slow relay calls exhaust every thread and starve the fast local endpoints of anywhere to run — see `docs/architecture.md`, "Federation worker relay pool separation". |
