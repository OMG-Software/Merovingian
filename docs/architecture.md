...
### Federation worker EDU relay

`FederationRuntimeState::edu_sink` is the same kind of injected hook as `pdu_sink` and `membership_acceptor` above, and until 0.10.20 it was the one hook the worker never relayed at all: `worker_event_loop.cpp` set `runtime.federation.edu_sink = {};` unconditionally, on the stated reasoning that "EDUs are ephemeral and acceptable to drop." That's true for `m.typing`, `m.receipt`, and `m.presence`, but not for `m.direct_to_device` — the EDU type that carries E2EE `m.room_key`/`m.room_key.withheld` to-device messages — nor, to a lesser extent, `m.device_list_update` (main's edu_sink implementation, wired via `wire_federation_callbacks_impl()` in `local_http_router.cpp`, records both into `PersistentStore` for the next `/sync`). Worse, an empty `edu_sink` isn't even logged as a drop: `inbound_request.cpp`'s transaction-processing loop treats "no `edu_sink` installed" as `edus_dispatched` (not `edus_dropped`), so the per-transaction `transaction.accepted` diagnostic — and the `200` returned to the sending server — gave no indication anything was lost.

Reproduced against a real two-homeserver federation (`matrix.ping.me.uk` → `pong.ping.me.uk`, `federation.worker.shards=2`, the shipped example config): a client on `matrix.ping.me.uk` sent a megolm-encrypted room message; the corresponding `m.room_key` to-device share, delivered as an `m.direct_to_device` EDU in the same or an adjacent `/send` transaction, landed on a worker shard and was dropped. The recipient's client (`pong.ping.me.uk`) logged `Can't find the room key to decrypt the event, withheld code: None` — not even a `m.room_key.withheld` notice, since that too is carried as an EDU and would have been dropped the same way — and repeatedly, fruitlessly, checked key backup for the missing session, which was never going to have it either, since the key was never received by any of the recipient's devices in the first place.

Fix: `worker_event_loop.cpp` now overrides `edu_sink` the same way it overrides `pdu_sink`/`membership_acceptor` — serialize the `InboundEduEnvelope` (edu_type, origin, content_json) into a new `edu_ingest` IPC frame and call main, rather than handling (or silently discarding) it locally. `content_json` is embedded as an escaped JSON string value via `ipc::ipc_json_str`, the same technique `pdu_ingest`'s `json` field already uses for arbitrary nested PDU content, so an EDU payload that happens to contain a literal `"type"` key (e.g. `m.room_key.withheld` content does) cannot be mistaken for the outer frame's own `"type"` field. `worker_pool.cpp`'s `handle_edu_ingest_request()` (a free function alongside `handle_membership_ingest_request()`, for the same test-seam reason) receives it on main's side, reconstructs the envelope via `federation::parse_inbound_edu_envelope()` (rejecting anything that wouldn't have parsed if handled directly by main), and invokes main's own unmodified `edu_sink` under `runtime.mutex`. No separate `sync_notifier->publish()` call is needed here either — every case inside main's `edu_sink` (typing, receipt, presence, direct_to_device, device_list_update) already publishes on its own success path. Typing/receipt/presence EDUs are relayed too rather than special-cased back to a local no-op, keeping this a single uniform path instead of one that silently regresses again if a future EDU type is added without updating a drop-list.

**Ordinary (non-membership) traffic never refreshed a worker's room snapshot either.** `notify_room_changed()`/`reload_room()` above closes the gap for the 7 membership-mutating room_service calls, but `send_event()` (ordinary messages and state changes) was never in that set, and `pdu_sink`'s worker override deliberately does not write to the worker's own store either ("does NOT write events", `worker_event_loop.cpp`). So a message sent locally through `send_event()`, or relayed inbound through a worker's `pdu_sink` call to main, reached only main's `PersistentStore` — a worker's own copy of that room's events never advanced except at the next membership-triggered `reload_room()`, which reloads everything (including messages) as a side effect but is only reachable through membership churn. A room that changed hands rarely after creation but kept receiving ordinary messages could therefore have a worker permanently serving `backfill_provider`/`event_query_provider`/`state_query_provider`/`state_ids_query_provider`/`missing_events_query_provider` responses missing every message sent since the last membership change — not a security break, but a real correctness gap for federated history in active rooms. Fix: `send_event()` (`room_service.cpp`) now also calls `notify_room_changed(runtime, room_id)`, matching the placement convention of the other 7 call sites (immediately after the `sync_notifier->publish()` block). For the inbound-relay direction, `worker_pool.cpp`'s `pdu_ingest` IPC handler now calls `this->notify_room_changed(env.room_id)` after a successful `pdu_sink` commit — pushing the just-committed event back down to whichever shard owns the room, which in practice is the same worker that made the `pdu_ingest` call in the first place, since `federation_worker_shard_for()` is a pure function of `room_id`. New `send_event notifies the federation worker when an ordinary message is sent` scenario in `tests/unit/test_homeserver_room_service.cpp` extends the existing `test_room_changed_log`-pinned contract to this eighth call site.

### Federation worker invite relay

`invite_handler` (backs `PUT /_matrix/federation/{v1,v2}/invite`) is not among the hooks the worker overrides — `pdu_sink`, `membership_acceptor`, and `edu_sink` are, but `invite_handler` was left running its default implementation (`local_http_router.cpp`) unmodified inside whichever process handled the request. That default persists the invite's membership row (`upsert_membership`), invite metadata (`database::upsert_invite`), and the invite event itself (`database::store_event_with_state`) directly into `rt->database.persistent_store`, then calls `sync_notifier->publish()`. `PUT /invite` is room-scoped (`room_endpoint_prefixes()` includes both `/_matrix/federation/v1/invite/` and `/_matrix/federation/v2/invite/`), so with `federation.worker.shards >= 1` (the shipped example config) it is handled by a worker — the exact same class of gap `membership_acceptor` had before 0.10.19, just for the invite endpoint instead of send_join/send_leave/send_knock. Since real clients only ever `/sync` against main, and nothing else ever re-syncs invite data into main's store, a remote server inviting one of this server's local users to a room hosted elsewhere was silently swallowed: the invite never appeared in the invited user's own `/sync`, with no error anywhere to explain why (the worker's own `sync_notifier->publish()` call is a no-op in user-visible terms too, since no real client's `/sync` ever terminates at a worker process).

Fix: `worker_event_loop.cpp` now overrides `invite_handler` the same way it overrides `membership_acceptor` — serialize the `InviteRequest` (room_id, event_id, room_version, invite_event_json, invite_room_state_json) into a new `invite_ingest` IPC frame and call main, rather than persisting locally. `invite_event_json` and each `invite_room_state_json` entry are embedded as escaped JSON string values via `ipc::ipc_json_str`, the same technique `pdu_ingest`'s `json` field already uses for arbitrary nested event content. `worker_pool.cpp`'s `handle_invite_ingest_request()` (a free function alongside `handle_membership_ingest_request()`, for the same test-seam reason) receives it on main's side and invokes main's own unmodified `invite_handler` under `runtime.mutex`; no separate `sync_notifier->publish()` call is needed, since the default `invite_handler` already does that itself on success. Deserializing the `invite_ingest_result` reply reuses `deserialize_membership_ingest_result`'s full-JSON-parse approach (`field_bool`/`field_int`/`field_string`) rather than a substring scanner, since `accepted`/`status` are raw bool/int fields `ipc::ipc_json_get_str` (a quote-terminated string extractor) cannot parse at all, and `signed_event_json` can carry arbitrary nested event content. New `handle_invite_ingest_request persists a worker-relayed federated invite to main's own store` scenario in `tests/integration/test_federation_worker_flow.cpp` drives the handler directly with a request shaped like a real worker's `invite_ingest` frame and asserts the invite's membership row and invite metadata land in main's own `PersistentStore` — the same test-seam pattern the membership fix established, for the same reason (a real end-to-end round trip needs a live, network-resolvable remote signing key to clear the federation-policy gate ahead of `invite_handler`).

### Federation worker one-time-key claim relay

`one_time_keys_claim_provider` (backs `POST /_matrix/federation/v1/user/keys/claim`) has a different shape of problem from the write-relay gaps above: `database::claim_one_time_key` already issues a real `DELETE FROM one_time_keys ...` against the shared database regardless of which process runs it, so the claim *is* durably recorded — the bug is that the *decision* of whether a key is still available is made by searching the in-memory `PersistentStore::one_time_keys` vector first (`std::ranges::find_if(store.one_time_keys, ...)`, `persistent_store.cpp`), and that vector is a per-process snapshot that is never invalidated by a different process's write to the same underlying table. `POST /user/keys/claim` is a non-room-scoped route, so in steady state it always lands on shard 0's worker — but `WorkerPool::handle()` falls back to running the request in-process on main when the owning shard is unhealthy (there is already a test scenario for this fallback). If shard 0 claims (and deletes) a key, and a later claim for the same device/algorithm happens to fall back to main while main's in-memory copy hasn't independently learned the key is gone, main can hand out the *same* one-time private key material a second time — reusing an Olm one-time prekey, which exists specifically to be used exactly once per session establishment; reuse undermines the forward-secrecy/authentication properties the handshake relies on. The same staleness also runs the other way for availability: one-time keys uploaded through main (via client-server `PUT /keys/upload`, which only ever touches main's own store) after a worker started are invisible to that worker's snapshot, so claims routed to it can dry up once its startup-time batch is exhausted even though the user's client keeps replenishing keys.

Fix: rather than patch the claim-and-delete to be atomic at the database layer (which would still leave main's own in-memory cache diverged from a worker's writes, just with a narrower race window), `one_time_keys_claim_provider` is relayed through main entirely, the same as `pdu_sink`/`membership_acceptor`/`edu_sink`/`invite_handler` — removing the split-brain structurally rather than mitigating it. `worker_event_loop.cpp` overrides `one_time_keys_claim_provider` to serialize the raw federation request body into a new `otk_claim_ingest` IPC frame and call main; the body is embedded as an escaped JSON string (`ipc::ipc_json_str`) so main's own provider parses it exactly as if it had received the federation request directly. `worker_pool.cpp`'s `handle_otk_claim_ingest_request()` receives it and invokes main's own unmodified `one_time_keys_claim_provider` under `runtime.mutex`, returning the response body (an empty string on failure, matching the provider's existing "empty return signals failure" contract) inside an `otk_claim_ingest_result` frame. New `handle_otk_claim_ingest_request decides a one-time-key claim against main's own store` scenario in `tests/integration/test_federation_worker_flow.cpp` seeds a one-time key, drives the handler with a request shaped like a real worker's `otk_claim_ingest` frame, and asserts both that the response echoes the claimed key and that the key is gone from main's own `PersistentStore::one_time_keys` afterward.

### Federation worker user/device/profile/event query relay

`user_devices_provider` (backs `GET /_matrix/federation/v1/user/devices/{userId}`), `device_keys_query_provider` (backs `POST /_matrix/federation/v1/user/keys/query`), and `profile_query_provider` (backs `GET /_matrix/federation/v1/query/profile`) share a failure mode distinct from every relay above. The write-relay hooks (`pdu_sink`, `membership_acceptor`, `edu_sink`, `invite_handler`) and the split-brain hook (`one_time_keys_claim_provider`) all involve a write that must land on main's single authoritative store. These three are pure reads, so there is no write to lose — the bug is that their default implementations (`local_http_router.cpp`) read `PersistentStore::device_keys` and `PersistentStore::profiles`, both per-process in-memory snapshots hydrated once at worker startup from the database and never refreshed afterward by a write another process makes. Unlike room-scoped reads (`event_query_provider`, `state_query_provider`, `state_ids_query_provider`, `backfill_provider`, `missing_events_query_provider`), which stay correct because `notify_room_changed()`/`reload_room()` re-reads the affected room whenever it changes (see "Federation worker room staleness" above), none of these three routes carry a room ID at all — they are keyed by user ID, and `room_endpoint_prefixes()` has no (and can have no) entry for them, so they always route to shard 0 with no notification channel that could ever refresh them. A device key uploaded, or a profile changed, through main's client-server API after a worker started is therefore permanently invisible to that worker, not just briefly stale.

Reproduced against a real two-homeserver federation (`matrix.ping.me.uk` → `pong.ping.me.uk`, `federation.worker.shards=2`, the shipped example config): `matrix.ping.me.uk` queried `GET /_matrix/federation/v1/user/devices/@james:pong.ping.me.uk` to learn which device to target an `m.room_key` to-device share at before sending a megolm-encrypted room message, and got a spurious `404 M_NOT_FOUND` because the query landed on a worker whose `device_keys` snapshot predated that device's key upload. With no current device list to target, the room key was never delivered; the recipient's client logged `Can't find the room key to decrypt the event, withheld code: None` for every message encrypted under that session, with nothing in the server's own logs indicating a failure — the transaction carrying the encrypted PDU itself was still accepted and delivered normally, since PDU delivery and to-device key delivery are entirely separate concerns.

Fix: `worker_event_loop.cpp` now overrides all three providers the same way it overrides `one_time_keys_claim_provider` — serialize the request (a bare `user_id` for `user_devices_provider`/`profile_query_provider`, the raw request body for `device_keys_query_provider`) into a new `user_devices_ingest` / `device_keys_query_ingest` / `profile_query_ingest` IPC frame and call main, rather than deciding locally. `worker_pool.cpp`'s `handle_user_devices_ingest_request()`/`handle_device_keys_query_ingest_request()`/`handle_profile_query_ingest_request()` (free functions alongside the other `*_ingest_request` handlers, for the same test-seam reason) receive them on main's side and invoke main's own unmodified providers under `runtime.mutex`. `profile_query_ingest_result` carries `found`/`displayname`/`avatar_url` as a real JSON object (parsed via `field_bool`/`field_string`, the same full-JSON-parse approach `deserialize_membership_ingest_result` uses) rather than `ipc::ipc_json_get_str`, since `found` is a raw bool field a quote-terminated string extractor cannot parse; the other two reuse the plain `response_body` string frame `otk_claim_ingest_result` established. New scenarios in `tests/integration/test_federation_worker_flow.cpp` seed a device key, a profile, and a one-time key respectively on main's own store, drive each handler with a request shaped like a real worker's ingest frame, and assert the response reflects main's data rather than an empty worker-local snapshot.

**`event_query_provider` has a related but structurally different problem, also closed here.** `directory_query_provider` (`query/directory`), `membership_template_provider` (`make_join`/`make_leave`/`make_knock`), `backfill_provider`, `missing_events_query_provider`, `state_query_provider`, and `state_ids_query_provider` are unaffected by the reasoning above where they are room-scoped (the last four), since `reload_room()` already keeps them correct. `event_query_provider` (`GET /event/{eventId}`) is the one room-scoped-in-principle exception: unlike `state`/`state_ids`/`backfill`/`get_missing_events`, its path carries only an event ID, not a room ID, so `room_endpoint_prefixes()` cannot route it by room and it always lands on shard 0 regardless of which shard actually owns the room the event belongs to — the same structural problem `query/directory` has (see "Shard routing must key on the same room ID string as the notification" above), not the "no notification channel exists at all" problem `user_devices_provider`/`device_keys_query_provider`/`profile_query_provider` have. A real fix for the routing misalignment itself would need an event-ID-to-room-ID index reachable before shard selection, which does not exist today (the same kind of real design decision `query/directory`'s alias-vs-room_id mismatch needs, not a one-line patch). Rather than build that index, `event_query_provider` is relayed through main via a new `event_query_ingest` IPC frame the same way the three providers above are — main receives every event via `pdu_sink` regardless of which shard accepted it, so answering from main's store is correct no matter which shard the request happened to land on. `worker_pool.cpp`'s `handle_event_query_ingest_request()` follows the same free-function/test-seam pattern as the other `*_ingest_request` handlers. New scenario in `tests/integration/test_federation_worker_flow.cpp` seeds an event on main's own store and asserts the relayed response contains it.
...
Every `merovingian-fed-worker` process ran a single fixed-size `net::ThreadPool` (`federation.worker.threads`, shipped default 4) for both jobs at once: dispatching every incoming `fed_request` forwarded from main, and — inside that same dispatch — synchronously blocking on `channel_ptr->send_request(...)` whenever the request needed `pdu_sink`, `edu_sink`, `membership_acceptor`, `invite_handler`, or one of the query-provider relays documented above (`profile_query_provider`, `device_keys_query_provider`, `user_devices_provider`, `event_query_provider`, `one_time_keys_claim_provider`). Those calls wait on main across a `std::recursive_mutex runtime.mutex` shared by every relayed operation from every shard, with timeouts of 10–60 seconds. If enough of them land on one shard concurrently to occupy all `threads` pool workers at once, the pool's queue (unbounded, per `net::ThreadPool`) has nowhere to run a newly arrived `fed_request` — even one that would never need to call main at all, such as `GET /_matrix/key/v2/server`'s worker-side counterpart or a `state`/`backfill`/`state_ids` read answered entirely from the worker's own local snapshot. The shard appears fully hung to main (and to the remote peer proxied through it) for as long as the backlog takes to drain: no crash, no restart, nothing in the logs beyond the eventual `WorkerPool::handle()` timeout at `federation.worker.request_timeout_seconds` (shipped default 30s) producing a `503` main sends the peer, who typically retries and succeeds once the backlog clears. Reproduced in production as a federation worker shard going unresponsive for roughly the configured 30-second timeout window with no crash or supervisor restart logged, coinciding with a burst of `503`s a remote peer retried successfully. The same mechanism plausibly explains intermittent to-device delivery gaps too: an EDU's own `edu_sink` call queued behind this backlog can complete so late that nothing is still listening for it (a client's `/sync` long-poll window, or the sending server's own retry patience), which looks indistinguishable from the message having been lost outright.

`threads` is sized as if it gated CPU-bound work ("increase for deployments that federate with many rooms simultaneously" — the pre-existing config comment), but the blocking-on-main/blocking-on-remote calls it also has to host are I/O-bound: a thread spends nearly all of that time parked on a condition variable, not consuming CPU, so sizing generously costs little. Fix: `worker_event_loop.cpp` now runs two pools. `local_pool` (sized by the existing `threads`) handles `fed_request`s for endpoints that answer entirely from the worker's own local, room-scoped snapshot with no relay: `make_join`/`make_leave`/`make_knock`, `backfill`, `query/directory`, `state`, `state_ids`, `get_missing_events`, `hierarchy`, and the no-op `send_edu` stub. `relay_pool` (new `federation.worker.relay_threads`, default 32) handles `fed_request`s for the ten endpoints that can call main (`transaction`, `send_join`/`send_leave`/`send_knock`, `invite`, `query_profile`, `query_keys`, `claim_keys`, `query_user_devices`, `query_event`) plus every `outbound_http_request` message (blocks on a remote server's response, the same class of wait). The classification is `federation::federation_endpoint_requires_main_relay(FederationEndpoint)` (`federation/transactions.hpp`), a pure function of the already-existing route-matching enum, applied to the result of `match_federation_route()` before a thread is committed — so the split costs one cheap JSON-and-route-match call per request, done on the IPC reader thread the same way request-type dispatch already was.

New `tests/unit/test_federation_transactions.cpp` scenario iterates every entry in `federation_routes()` and asserts `federation_endpoint_requires_main_relay()` places it in the expected bucket, so a future endpoint added to `FederationEndpoint` without an explicit classification is caught immediately (the switch has no `default:` case). A full end-to-end proof of the starvation fix — saturate `relay_pool` on a real spawned worker subprocess and show a concurrent local-only request still completes quickly — is not included: reaching any relay-requiring endpoint through a real worker subprocess first requires clearing the "remote is unknown" federation-policy gate ahead of it, which needs a live, network-resolvable remote signing key; this is the same orthogonal-infrastructure gap the EDU, invite, membership, OTK-claim, and query-provider relay fixes above all hit and none of them solved either — their scenarios all call the relevant `handle_*_ingest_request()` free function directly instead, which proves the relay logic but, like this fix, cannot exercise the worker process's own thread pools.
