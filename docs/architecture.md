...
5. Unlock the stripe and commit the prepared statements with `database::commit_persistent_transaction()`, so commits for different rooms can overlap.
6. Re-lock the stripe and call `database::apply_store_event_with_state()` to mirror the committed rows into the in-memory vectors.
