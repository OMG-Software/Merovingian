...
**E2EE keys** (`federation/key_query.hpp`, `key_signatures.hpp`): `key_query` builds the inbound `/user/keys/query`, `/user/keys/claim` and `/user/devices/{userId}` responses and the `m.device_list_update` / `m.signing_key_update` EDU contents, and filters remote `/user/keys/query` responses for the client `/keys/query` proxy (`accept_remote_key_query_response()`). `key_signatures` is the one place stored `/keys/signatures/upload` signatures are merged into a published key, with the per-viewer visibility rule from [ADR-0060](adr/0060-show-an-uploaded-key-signature-only-to-its-owner-audience.md). The client path in `homeserver/client_server.cpp` uses the same helpers, so what a local client and a remote server see cannot drift apart. Remote users' keys are proxied, never cached ([ADR-0061](adr/0061-proxy-remote-cross-signing-keys-rather-than-caching-them.md)).
