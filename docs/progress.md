...
| E2EE key APIs | `runtime-wired` | Key API route/planning boundary, authenticated client-server runtime dispatch for upload/query/claim/cross-signing/signature/backup route shapes, server-blind payload redaction, audit records, and BDD runtime coverage | Persist device keys, one-time keys, fallback keys, cross-signing keys, signatures, key backups, and device-list streams; add full Matrix v1.18 semantics and conformance fixtures. |
