...
{
    return {std::move(method), std::move(path_template), true, requires_admin, {20U, 60U}};
}
...
auto policy_server_hook_allows(PolicyServerHook const& hook) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::account, enforcement_reason("policy_server_timeout_unconfigured", "policy check unavailable", "policy server timeout is zero"));
    }
    if (!hook.reachable && !hook.allow_without_result)
    {
        return deny(PolicySurface::account, enforcement_reason("policy_server_unreachable", "policy check unavailable", "policy server was unreachable"));
    }
    if (hook.rule_id.empty() && !hook.allow_without_result)
    {
        return deny(PolicySurface::account, enforcement_reason("policy_server_no_result", "policy check unavailable", "policy server returned no rule id"));
    }
...
{
    if (!matrix_id_is_valid(request.sender_user_id) || !matrix_id_is_valid(request.target_user_id) || request.room_id.empty())
    {
        return deny(PolicySurface::invite, enforcement_reason("invalid_invite_policy_input", "invite cannot be processed", "invite policy input is incomplete"));
    }
    if (request.blocked_by_local_policy)
    {
        return deny(PolicySurface::invite, enforcement_reason("invite_blocked", "invite blocked by server policy", "local invite policy blocked the request"));
    }
...
auto evaluate_registration_policy(RegistrationPolicyRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::registration, enforcement_reason("invalid_registration_policy_input", "registration cannot be processed", "registration policy input is incomplete"));
    }
    if (!request.registrations_enabled)
    {
        return deny(PolicySurface::registration, enforcement_reason("registration_disabled", "registration is disabled", "local registration policy is disabled"));
    }
    if (request.blocked_by_local_policy)
    {
        return deny(PolicySurface::registration, enforcement_reason("registration_blocked", "registration blocked by server policy", "local registration policy blocked the request"));
    }
...
auto evaluate_account_policy(AccountPolicyRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::account, enforcement_reason("invalid_account_policy_input", "account cannot be processed", "account policy input is incomplete"));
    }
    if (request.enforcement.suspended)
    {
        return deny(
            PolicySurface::account,
...
auto evaluate_federation_policy(FederationPolicyRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::federation, enforcement_reason("invalid_federation_policy_input", "federation request cannot be processed", "federation policy input is incomplete"));
    }
    if (request.held_for_review)
    {
        return review(PolicySurface::federation, enforcement_reason("federation_held_for_review", "federation request held for review", "federation origin is held for review"));
    }
    if (request.blocked_by_local_policy)
    {
        return deny(PolicySurface::federation, enforcement_reason("federation_blocked", "federation blocked by server policy", "local federation policy blocked the origin"));
    }
...
auto evaluate_media_policy(MediaPolicyRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::media, enforcement_reason("invalid_media_policy_input", "media cannot be processed", "media policy input is incomplete"));
    }
    if (request.held_for_review)
    {
        return review(PolicySurface::media, enforcement_reason("media_held_for_review", "media held for review", "media item is held for review"));
    }
    if (request.blocked_by_local_policy)
    {
        return deny(PolicySurface::media, enforcement_reason("media_blocked", "media blocked by server policy", "local media policy blocked the item"));
    }
...
auto evaluate_room_policy(RoomPolicyRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::room, enforcement_reason("invalid_room_policy_input", "room cannot be processed", "room policy input is incomplete"));
    }
    if (request.held_for_review)
    {
        return review(PolicySurface::room, enforcement_reason("room_held_for_review", "room held for review", "room is held for review"));
    }
    if (request.blocked_by_local_policy)
    {
        return deny(PolicySurface::room, enforcement_reason("room_blocked", "room blocked by server policy", "local room policy blocked the room"));
    }
...
auto account_enforcement_statements(AccountEnforcementRecord const& record)
    -> std::vector<database::PreparedStatement>
{
    ...
    return {
        {
            "trust_safety_update_account_enforcement",
            "UPDATE accounts SET suspended = $1, locked = $2, enforcement_reason = $3 WHERE user_id = $4",
            {
...
auto review_policy(ReviewRecord const& record) -> PolicyDecision
{
    ...
    {
        return deny(surface, enforcement_reason("invalid_review_target", "target cannot be processed", "review target is invalid"));
    }
    if (!record.active)
    {
...
auto match_reporting_api_route(std::string_view method, std::string_view target)
...
auto validate_safety_report(SafetyReportRequest const& request) -> PolicyDecision
{
    ...
    {
        return deny(PolicySurface::room, enforcement_reason("invalid_report", "report cannot be processed", "report is missing reporter, room, or event"));
    }
    if (request.reason.empty())
    {
        return deny(PolicySurface::room, enforcement_reason("report_reason_required", "report reason is required", "report reason is empty"));
    }
    if (request.score < -100 || request.score > 100)
    {
        return deny(PolicySurface::room, enforcement_reason("report_score_out_of_range", "report score is invalid", "report score must be between -100 and 100"));
    }

    return {true, PolicySurface::room, PolicyAction::accept_report, {}, false, {}};
}

auto make_safety_audit_event(
...
{
    return {
        std::string{"trust_safety."} + policy_surface_name(decision.surface) + '.' + policy_action_name(decision.action),
        std::string{actor},
        std::string{entity},
        decision.surface,
        decision.action,
...
{
    return event.event_type + " actor=" + event.actor + " entity=" + event.entity
