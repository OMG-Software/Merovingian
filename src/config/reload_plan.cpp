...
auto build_reload_plan(Config const& current, Config const& next) -> ReloadPlan
{
    ...
    if (current.security().encryption.require_for_direct_messages != next.security().encryption.require_for_direct_messages)
    {
    ...
    if (current.security().encryption.allow_unencrypted_public_rooms != next.security().encryption.allow_unencrypted_public_rooms)
    {
        add_change(plan, "security.encryption.allow_unencrypted_public_rooms");
    }
    if (
        current.security().encryption.block_unencrypted_federated_private_rooms
