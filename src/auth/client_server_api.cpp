...
namespace
{
    ...
    [[nodiscard]] auto route(std::string method, std::string path_template, ClientAuthEndpoint endpoint)
        -> ClientAuthRoute
    {
    ...
    [[nodiscard]] auto select_device_statement(std::string_view user_id, std::string_view device_id)
    ...
    [[nodiscard]] auto insert_device_statement(std::string_view user_id, std::string_view device_id)
        -> database::PreparedStatement
    {
        return {
    ...
    [[nodiscard]] auto persist_token_hash_statement(std::string_view user_id, std::string_view device_id,
                                                    TokenHash const& token_hash) -> database::PreparedStatement
    {
        return {
    ...
    [[nodiscard]] auto revoke_device_token_statement(std::string_view user_id, std::string_view device_id)
        -> database::PreparedStatement
    {
        return {
    ...
    [[nodiscard]] auto update_device_statement(std::string_view user_id, std::string_view device_id)
        -> database::PreparedStatement
    {
        return {
    ...
    [[nodiscard]] auto delete_device_statement(std::string_view user_id, std::string_view device_id)
        -> database::PreparedStatement
    {
        return {
...
auto client_auth_database_statements(ClientAuthEndpoint endpoint, std::string_view user_id, std::string_view device_id,
                                     TokenHash const& token_hash) -> std::vector<database::PreparedStatement>
{
    switch (endpoint)
    {
    case ClientAuthEndpoint::login:
        return {select_account_statement(user_id), insert_device_statement(user_id, device_id),
                persist_token_hash_statement(user_id, device_id, token_hash)};
    ...
    case ClientAuthEndpoint::register_account:
        return {insert_device_statement(user_id, device_id),
                persist_token_hash_statement(user_id, device_id, token_hash)};
    ...
    case ClientAuthEndpoint::delete_device:
        return {select_device_statement(user_id, device_id), delete_device_statement(user_id, device_id),
                revoke_device_token_statement(user_id, device_id)};
    }

    return {};
}

auto make_client_auth_boundary_plan(ClientAuthRoute const& route, std::string_view user_id, std::string_view device_id,
                                    TokenHash const& token_hash, bool allowed, std::string_view reason)
