...
auto handle_client_server_request(ClientServerRuntime& rt, LocalHttpRequest const& req, bool can_wait) -> DispatchResult
{
    ...
    if (req.method == "POST" && req.target == "/_matrix/client/v3/createRoom")
    {
        ...
        // homeserver-only path. The client-server handler overrides join_rules
        // here to apply the preset (e.g. "public" for public_chat), then adds
        // history_visibility which the lower-level create_room does not emit.
