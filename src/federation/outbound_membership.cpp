                                   ...
                                   std::string_view signed_event_json) -> OutboundTransaction
{
    ...
    return make_outbound_transaction(destination, "PUT", target, origin, signed_event_json);
}
                          ...
                          std::vector<std::string> const& invite_room_state_json) -> OutboundTransaction
{
    if (room_version.empty())
    {
        ...
        target.push_back('/');
        target.append(event_id);
        return make_outbound_transaction(destination, "PUT", target, origin, signed_invite_event_json);
    }
    ...
    return make_outbound_transaction(destination, "PUT", target, origin, body);
}
