...
[[nodiscard]] auto contains_signature_for(
...
) noexcept -> bool
{
    return std::ranges::any_of(signatures, [expected_server](events::EventSignature const& signature) {
        return signature.server_name == expected_server && !signature.key_id.empty() && !signature.signature.empty();
    });
}
...
auto verify_federation_request_signature(FederationRequestSignature const& signature)
...
auto verify_federation_event_signatures(
    std::vector<events::EventSignature> const& signatures,
    std::string_view expected_server
) -> FederationVerificationDecision
{
...
{
    return {
        "SSRF boundary: never connect to loopback, link-local, private, or unique-local remote addresses discovered through federation resolution.",
        "DNS rebinding boundary: bind the validated address set to the outbound request and reject host/address drift between discovery and connect.",
        "Trust boundary: request signatures, event signatures, remote rate limits, backoff, circuit breakers, reputation, and quarantine must be evaluated before accepting federation traffic.",
    };
}
